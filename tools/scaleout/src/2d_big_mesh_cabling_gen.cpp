...
InputConfig parse_arguments(int argc, char** argv) {
    ...
    options.add_options()(
        ...
        cxxopts::value<std::string>())(
        "y,galaxy-type",
        "Galaxy type: 'WH_GALAXY' or 'BH_GALAXY'",
    ...
    try {
        ...
        if (config.nodes_1 % 4 != 0) {
            throw std::invalid_argument(
                ...
                ") must be divisible by 4 for 8x4 Galaxy basis");
        }
...
int main(int argc, char** argv) {
    try {
        ...
        std::string output_path = "out/scaleout/" + std::string(config.galaxy_type) + "_" + "big_mesh_" +
                                  std::to_string(config.nodes_0) + "x" + std::to_string(config.nodes_1) + "_" +
                                  config.topology + ".textproto";
