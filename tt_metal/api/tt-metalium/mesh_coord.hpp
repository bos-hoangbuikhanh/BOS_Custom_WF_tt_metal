...
public:
    ...
    // Note: End iterators have undefined coordinates and should not be dereferenced, following C++ iterator
    // conventions.
    class Iterator {
    public:
...
MeshContainer<T>::Iterator::Iterator(
    ...
    value_proxy_(
        coord_iter_ != container_->coord_range_.end() ? *coord_iter_
                                                      : MeshCoordinate::zero_coordinate(container->shape().dims()),
...
typename MeshContainer<T>::Iterator& MeshContainer<T>::Iterator::operator++() {
    ...
    value_proxy_ = ValueProxy(
        coord_iter_ != container_->coord_range_.end() ? *coord_iter_
                                                      : MeshCoordinate::zero_coordinate(container_->shape().dims()),
        linear_index_ < container_->values_.size() ? &container_->values_[linear_index_] : nullptr);
    return *this;
}
