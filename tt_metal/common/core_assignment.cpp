...
std::vector<CoreCoord> get_optimal_dram_to_physical_worker_assignment(
    ...
    // Remove this once we have a proper way to get the optimal worker placement for WH and BH.
    std::vector<CoreCoord> dram_interface_workers;
    ...
    for (int i = 0; i < num_dram_banks; ++i) {
        ...
        if (arch == ARCH::WORMHOLE_B0 or arch == ARCH::BLACKHOLE) {
            if (i > 4) {
                dram_interface_workers.push_back(CoreCoord(0, 2));
            } else {
                dram_interface_workers.push_back(CoreCoord(i, 4));
            }
        }
    }
