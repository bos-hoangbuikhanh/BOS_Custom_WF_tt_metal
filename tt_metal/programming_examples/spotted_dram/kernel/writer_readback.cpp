...
void kernel_main() {
    ...
    constexpr uint32_t barrier_threshold = get_barrier_read_threshold<tile_bytes, 20>();
    uint32_t barrier_count = 0;
    uint32_t curr_tile_id = start_id;
    cb_wait_front(cb_id_in0, 7);
    uint32_t l1_write_addr = get_write_ptr(cb_id_in0);
    for (uint32_t h = 0; h < block_height_tiles; h++) {
        uint32_t tile_id = curr_tile_id;
        for (uint32_t w = 0; w < block_width_tiles; w++) {
            noc_async_write_tile(tile_id, s, l1_write_addr);
            tile_id++;
            l1_write_addr += tile_bytes;
            if (++barrier_count == barrier_threshold) {
                // noc_async_read_barrier();
                noc_async_write_barrier();
                cb_pop_front(cb_id_in0, 7);
                barrier_count = 0;
                cb_wait_front(cb_id_in0, 7);
            }
        }
