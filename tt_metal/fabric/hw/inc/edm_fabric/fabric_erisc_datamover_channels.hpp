...
template <typename HEADER_TYPE, uint8_t NUM_BUFFERS>
using EthChannelBuffer =
    typename ChannelBufferTypeSelector<USE_STATIC_SIZED_CHANNEL_BUFFERS, HEADER_TYPE, NUM_BUFFERS>::type;

template <typename HEADER_TYPE, uint8_t NUM_BUFFERS>
using SenderEthChannel =
    typename SenderChannelTypeSelector<USE_STATIC_SIZED_CHANNEL_BUFFERS, HEADER_TYPE, NUM_BUFFERS>::type;
...
template <
    ...
    FabricChannelPoolType PoolType,
    size_t NumBuffers,
    template <typename, size_t> class StaticType,
    typename ElasticType>
struct GenericChannelTypeSelector {
    using type =
        std::conditional_t<PoolType == FabricChannelPoolType::STATIC, StaticType<HEADER_TYPE, NumBuffers>, ElasticType>;
};
...
struct HeterogeneousChannelTuple {
    ...
    FORCE_INLINE void init(size_t buffer_size_bytes, size_t header_size_bytes) {
        init_impl<PoolCollection>(
            buffer_size_bytes, header_size_bytes, std::make_index_sequence<std::tuple_size_v<ChannelTypes>>());
    }
...
private:
    ...
    FORCE_INLINE void init_single_channel(Channel& chan, size_t buffer_size_bytes, size_t header_size_bytes) {
        ...
        constexpr size_t pool_idx = ChannelToPoolIndex[ChannelIdx];
        constexpr auto pool_type = static_cast<FabricChannelPoolType>(PoolCollection::channel_pool_types[pool_idx]);
