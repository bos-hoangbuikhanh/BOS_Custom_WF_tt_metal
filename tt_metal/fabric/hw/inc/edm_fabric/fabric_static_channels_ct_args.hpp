...
struct ElasticChannelPool : public CtArgConsumer<ElasticChannelPool<CT_ARG_IDX_BASE>> {
    ...
    static constexpr std::array<size_t, num_chunks> chunk_base_addresses =
        fill_array_with_next_n_args<size_t, CHUNKS_CT_ARGS_IDX_BASE, num_chunks>();

    static constexpr size_t GET_NUM_ARGS_CONSUMED() { return 2 + num_chunks * ARGS_PER_CHUNK; }
};
...
struct PoolsBuilderImpl<CT_ARG_IDX, NumPools, PoolTypesArray, CurrentIdx, RestIndices...> {
    ...
    using CurrentPoolType = typename std::conditional<
        ...
        ElasticChannelPool<CT_ARG_IDX>>::type;
    ...
    using type = decltype(std::tuple_cat(std::declval<std::tuple<CurrentPoolType>>(), std::declval<RestOfPools>()));

    // static constexpr size_t final_ct_arg_idx = RestOfPools::final_ct_arg_idx;
    static constexpr size_t final_ct_arg_idx =
        PoolsBuilderImpl<next_ct_arg_idx, NumPools, PoolTypesArray, RestIndices...>::final_ct_arg_idx;
};
...
struct ChannelPoolCollection
    ...
    static constexpr size_t pools_data_base_idx = channel_pool_types_base_idx +  // start of pool types
    ...
    static constexpr std::array<size_t, NumReceiverChannels> receiver_channel_to_pool_index =
        fill_array_with_next_n_args<
            size_t,
            sender_channel_to_pool_index_base_idx +
                NumSenderChannels,  // channel_pool_types_base_idx + num_channel_pools + NumSenderChannels,
            NumReceiverChannels>();

    static constexpr size_t GET_NUM_ARGS_CONSUMED() {
        return (sender_channel_to_pool_index_base_idx + NumSenderChannels + NumReceiverChannels) - CT_ARG_IDX_BASE;
    }
};
...
struct ChannelPoolLookup {
    ...
    // Final type alias
    using pools_tuple = typename BuildTuple<typename make_index_sequence<NumPools>::type>::type;
};
