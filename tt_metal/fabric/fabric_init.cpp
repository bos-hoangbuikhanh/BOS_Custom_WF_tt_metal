...
void build_tt_fabric_program(
    ...
    std::unordered_map<tt::tt_fabric::chan_id_t, std::unique_ptr<tt::tt_fabric::FabricRouterBuilder>>&
        router_builders) {
    ...
    for (const auto& [direction, remote_fabric_node_id] : chip_neighbors) {
        ...
        auto get_fabric_router_config = [&](bool is_dispatch_link, auto eth_direction) {
            ...
            return fabric_context.get_fabric_router_config(fabric_tensix_config, eth_direction);
        };
