...
void run_default_galaxy_optimizer(
    ...
    if (ctx.topology == Topology::Ring) {
        enable_noc_selection_opt = (ctx.num_links > ring_noc_selection_link_threshold) &&
                                   (edm_builder1.get_noc_y() != edm_builder2.get_noc_y());
    } else {
        enable_noc_selection_opt = (ctx.num_links > line_noc_selection_link_threshold) &&
                                   (edm_builder1.get_noc_y() != edm_builder2.get_noc_y());
    }
    log_debug(
        tt::LogTest,
