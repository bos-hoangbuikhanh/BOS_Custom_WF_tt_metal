...
bool WriteToDeviceDRAMChannel(
    ...
    uint64_t first_bank_base = device->allocator()->get_bank_offset(BufferType::DRAM, 0);
    uint64_t unreserved_base = device->allocator()->get_base_allocator_addr(HalMemType::DRAM);
    uint64_t unreserved_offset = unreserved_base - first_bank_base;
    TT_FATAL(
        address >= unreserved_offset,
        "Cannot write to reserved DRAM region, channel-relative addresses [0, {}) are reserved!",
        unreserved_offset);
...
// [BOS-ARCH-001] 64-bit address
bool ReadFromDeviceDRAMChannel(IDevice* device, int dram_channel, uint64_t address, std::span<uint8_t> host_buffer) {
    bool pass = true;
...
tt::tt_metal::IDevice* CreateDeviceMinimal(
    ChipId device_id, const uint8_t num_hw_cqs, const tt::tt_metal::DispatchCoreConfig& dispatch_core_config) {
    ZoneScoped;
    tt::tt_metal::MetalContext::instance().initialize(
        dispatch_core_config, num_hw_cqs, {}, DEFAULT_L1_SMALL_SIZE, true);
    auto* dev =
        new tt::tt_metal::Device(device_id, num_hw_cqs, DEFAULT_L1_SMALL_SIZE, DEFAULT_TRACE_REGION_SIZE, {}, true);
    tt::tt_metal::MetalContext::instance().get_cluster().set_internal_routing_info_for_ethernet_cores(true);
    return dev;
}

bool CloseDevice(tt::tt_metal::IDevice* device) {
    ZoneScoped;
    auto device_id = device->id();
