...
constexpr static std::uint64_t DRAM_BARRIER_BASE = 0x1200000000;
constexpr static std::uint64_t DRAM_BARRIER_SIZE =
    ((sizeof(uint32_t) + DRAM_ALIGNMENT - 1) / DRAM_ALIGNMENT) * DRAM_ALIGNMENT;

constexpr static std::uint64_t DRAM_PROFILER_BASE = DRAM_BARRIER_BASE + DRAM_BARRIER_SIZE;
#if defined(TRACY_ENABLE)
constexpr static std::uint32_t MAX_NUM_UNHARVESTED_TENSIX_CORES = 24;
constexpr static std::uint32_t MAX_NUM_ETH_CORES = 0;
constexpr static std::uint32_t MAX_NUM_CORES = MAX_NUM_UNHARVESTED_TENSIX_CORES + MAX_NUM_ETH_CORES;
constexpr static std::uint32_t NUM_DRAM_CHANNELS = 3;
...
constexpr static std::uint32_t DRAM_PROFILER_SIZE =
    2 * ((((PROFILER_FULL_HOST_BUFFER_SIZE_PER_RISC * MaxProcessorsPerCoreType * CEIL_NUM_CORES_PER_DRAM_CHANNEL) +
           DRAM_ALIGNMENT - 1) /
          DRAM_ALIGNMENT) *
         DRAM_ALIGNMENT);
#else
...
// [BOS-ARCH-004] memory system
constexpr static std::uint64_t DRAM_UNRESERVED_BASE = DRAM_PROFILER_BASE + DRAM_PROFILER_SIZE;

constexpr static std::uint64_t DRAM_UNRESERVED_SIZE = MEM_DRAM_SIZE - (DRAM_UNRESERVED_BASE - 0x1200000000);
