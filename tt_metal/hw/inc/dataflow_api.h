...
/**
 * This blocking call waits for all the outstanding enqueued *noc_async_write_barrier*
 * calls issued on the current Tensix core to complete. After returning from
 * this call the *noc_async_write_multicast* queue will be empty for the current Tensix
 * core.
 * Arch of BOS unexpectedly includes dram cores while broadcasting...
 *
 * Return value: None
 *
 * | Argument | Description                          | Type     | Valid Range | Required |
 ...
 */
FORCE_INLINE
void noc_async_multicast_write_barrier(uint8_t noc = noc_index) {
    RECORD_NOC_EVENT(NocEventType::WRITE_BARRIER_START);
    WAYPOINT("NWBW");
    // unlike noc_nonposted_writes_acked,  noc_nonposted_writes_num_issued only increases 1
    //  meaning that we could make a barrier using ncrisc_noc_nonposted_writes_sent
