...
constexpr cores_in_row noc2axi_noc0 = {3, {1, 2, 3}};
constexpr cores_in_row noc2axi_noc1 = {1, {0, 1, 5}};
constexpr cores_in_row noc2axi_per_noc[2] = {noc2axi_noc0, noc2axi_noc1};

// [BOS-ARCH-005] Support to count extra ack count
inline __attribute__((always_inline)) uint32_t
mcast_extra_ack_count(uint32_t noc_mcast_xy_coordinate, uint32_t noc = 0) {
    // 3 out of 6 Noc2AXI cores expose only the ring0 NoC NIU registers and
    // another 3 expose only the ring1 NoC NIU registers,
    // although all NoC2AXI cores have routers for both ring.
    // Therefore not able to set all NoC2AXI cores to be excluded from multicast.

    // When multicast through ring0, (1,3), (2,3), (3,3) cores will unexpectedly receive multicast and send-back acks
    // When multicast through ring1, (0,1), (1,1), (5,1) cores will unexpectedly receive multicast and send-back acks
    // So we should count number of these cores included in the multicast range and add it to the expected ack count

    uint32_t start_y = (noc_mcast_xy_coordinate >> (NOC_ADDR_NODE_ID_BITS * 3)) & 0b111111;
    uint32_t end_y = (noc_mcast_xy_coordinate >> (NOC_ADDR_NODE_ID_BITS * 1)) & 0b111111;

    if ((start_y <= end_y) ^ (start_y <= noc2axi_per_noc[noc].y) ^ (end_y >= noc2axi_per_noc[noc].y)) {
        uint32_t count = 0;
        uint32_t start_x = noc_mcast_xy_coordinate >> (NOC_ADDR_NODE_ID_BITS * 2) & 0b111111;
        uint32_t end_x = noc_mcast_xy_coordinate >> (NOC_ADDR_NODE_ID_BITS * 0) & 0b111111;

        for (auto x : noc2axi_per_noc[noc].x) {
            count += (start_x <= end_x) ^ (start_x <= x) ^ (end_x >= x);
        }
...
// [BOS-ARCH-003] HW architecture
// (40 bits of address followed by coordinates) This means that lo and mid registers need to have the address portion
...
constexpr uint32_t NOC_ADDR_COORD_SHIFT = 40;
...
constexpr uint32_t NOC_PCIE_MASK = 0xFF;
...
inline __attribute__((always_inline)) void ncrisc_noc_fast_write(
    ...
    // [BOS-ARCH-005] Support to count extra ack count
    if (mcast) {
        num_dests += mcast_extra_ack_count((uint32_t)(dest_addr >> NOC_ADDR_COORD_SHIFT) & NOC_COORDINATE_MASK, noc);
    }
...
inline __attribute__((always_inline)) void ncrisc_noc_fast_read_with_transaction_id(
    uint32_t noc, uint32_t cmd_buf, uint64_t src_base_addr, uint32_t src_addr, uint32_t dest_addr, uint32_t trid) {
    ...
    // [BOS-ARCH-009] Change mid register mask
    NOC_CMD_BUF_WRITE_REG(noc, cmd_buf, NOC_TARG_ADDR_MID, (uint32_t)(src_addr_ >> 32) & 0xFF);
    NOC_CMD_BUF_WRITE_REG(noc, cmd_buf, NOC_CMD_CTRL, NOC_CTRL_SEND_REQ);
