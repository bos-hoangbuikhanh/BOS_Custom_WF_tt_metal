...
// [BOS-ARCH-010] Add fast divide
inline __attribute__((always_inline)) uint32_t fast_udiv_24(uint32_t n)
{
    ...
    return (((uint64_t) n *  0xAAAAAAAB) >> 32) >> 4;
}
inline __attribute__((always_inline)) uint32_t fast_udiv_3(uint32_t n) {
    // Uses embedding style magic number
    // * fixed point 1/12 then shifting.
    // https://web.archive.org/web/20190703172151/http://www.hackersdelight.org/magic.htm
    return (((uint64_t)n * 0xAAAAAAAB) >> 32) >> 1;
}
inline __attribute__((always_inline)) uint32_t fast_udiv_6(uint32_t n) {
    // Uses embedding style magic number
    // * fixed point 1/12 then shifting.
    // https://web.archive.org/web/20190703172151/http://www.hackersdelight.org/magic.htm
    return (((uint64_t)n * 0xAAAAAAAB) >> 32) >> 2;
}
template <uint32_t d>
inline __attribute__((always_inline)) uint32_t udivsi3_const_divisor(uint32_t n) {
    ...
    if constexpr (d == 7) {
        ...
        return fast_udiv_3(n);
    } else if constexpr (d == 24) {
        return fast_udiv_24(n);
    } else if constexpr (d == 6) {
        return fast_udiv_6(n);
    }
