...
// Helper function to get DRAM NOC x-coordinate
// [BOS-ARCH-002] DRAM core assigement
// Avoids PCIE-assigned core (5,3) by distributing traffic to (0,3) and (4,3)
// USE_BANK_INDEX = false (default): Worker cores use my_x[0]-based allocation
// USE_BANK_INDEX = true: Dispatch cores use bank_index-based allocation
template <bool USE_BANK_INDEX = false>
FORCE_INLINE uint16_t get_dram_noc_x(uint32_t bank_index, uint8_t noc) {
    uint16_t x;
    if (noc == 0) {
        if constexpr (USE_BANK_INDEX) {
            // Dispatch core mode: Use bank_index to select DRAM core
            uint64_t noc_xy_orig = dram_bank_to_noc_xy[noc][bank_index % 3];
            x = (noc_xy_orig >> NOC_COORD_REG_OFFSET) & NOC_NODE_ID_MASK;
            if (x == 5) {
                x = (bank_index % 2 == 0) ? 0 : 4;
            }
        } else {
            // my_x[0] = {4,5,0} -> (0,3), my_x[0] = {1,2,3} -> (4,3)
            uint16_t t = my_x[0];
            uint16_t is_group0 = ((uint16_t)(t - 1) >= 3) ? 1 : 0;
            x = is_group0 ? 0 : 4;
        }
    } else {
        // Ring1: {4,5} -> (1,3), {0,2} -> (2,3), {1,3} -> (3,3)
        uint16_t t = my_x[0];
        uint16_t c4 = (t >= 4);
        uint16_t even = !(t & 1);
...
template <bool DRAM, bool USE_BANK_INDEX = false>
FORCE_INLINE uint64_t get_noc_xy(uint32_t bank_index, uint8_t noc = noc_index) {
    if constexpr (DRAM) {
        uint16_t x = get_dram_noc_x<USE_BANK_INDEX>(bank_index, noc);
        return ((DYNAMIC_NOC_Y(noc, 3) << NOC_ADDR_NODE_ID_BITS) | DYNAMIC_NOC_X(noc, x)) << NOC_COORD_REG_OFFSET;
    } else {  // L1
...
template <bool DRAM>
FORCE_INLINE uint64_t get_bank_offset(uint32_t bank_index) {
    if constexpr (DRAM) {  // DRAM
...
*/
FORCE_INLINE
std::uint64_t get_noc_addr_helper(std::uint32_t noc_xy, std::uint64_t addr) {
    /*
...
uint64_t get_system_memory_noc_addr(
    ...
    uint64_t addr = base_addr + page_size * id + offset;
...
struct InterleavedAddrGen {
    ...
    std::uint64_t get_noc_addr(const uint32_t id, const uint32_t offset = 0, uint8_t noc = noc_index) const {
        ...
        uint64_t addr = this->get_addr(id, bank_offset_index, bank_index, offset);
        uint64_t noc_xy = interleaved_addr_gen::get_noc_xy<DRAM, USE_BANK_INDEX>(bank_index, noc);
        uint64_t noc_addr = get_noc_addr_helper(noc_xy, addr);
        // DPRINT << "bank_offset_index :" <<bank_offset_index <<ENDL();
        // DPRINT << "bank_index :" <<  bank_index <<ENDL();
        // DPRINT << "noc_xy :" << HEX() << noc_xy <<ENDL();
        // DPRINT << "noc_addr :" << HEX() << noc_addr <<ENDL();
        return noc_addr;
    }
};
...
struct InterleavedPow2AddrGen {
    const uint64_t bank_base_address;
    static constexpr bool is_dram = DRAM;
...
template <bool DRAM, uint32_t tile_hw = 1024, bool USE_BANK_INDEX = false>
...
// clang-format on
template <bool DRAM>
FORCE_INLINE uint64_t get_noc_addr_from_bank_id(uint32_t bank_id, uint64_t bank_address_offset, uint8_t noc = noc_index) {
    uint64_t noc_addr;
    if constexpr (DRAM) {
        uint16_t x = interleaved_addr_gen::get_dram_noc_x<false>(bank_id, noc);
        noc_addr = (DYNAMIC_NOC_Y(noc, 3) << NOC_ADDR_NODE_ID_BITS) | DYNAMIC_NOC_X(noc, x);
        bank_address_offset += interleaved_addr_gen::get_bank_offset<DRAM>(bank_id);
    } else {
