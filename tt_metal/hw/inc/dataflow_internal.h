...
void noc_fast_read_set_src_xy(uint64_t src_addr) {
    ...
    NOC_CMD_BUF_WRITE_REG(noc_index, NCRISC_RD_CMD_BUF, NOC_TARG_ADDR_MID, (uint32_t)(src_addr >> 32) & 0xFF);
#endif
...
#ifdef ARCH_BLACKHOLE
    // [BOS-ARCH-009] Change mid register mask
    NOC_CMD_BUF_WRITE_REG(noc_index, NCRISC_WR_CMD_BUF, NOC_RET_ADDR_MID, (uint32_t)(dest_addr >> 32) & 0xFF);
#endif
    NOC_CMD_BUF_WRITE_REG(
        noc_index,
        NCRISC_WR_CMD_BUF,
