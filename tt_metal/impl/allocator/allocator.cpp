...
void Allocator::init_one_bank_per_channel() {
    ...
    const metal_SocDescriptor& soc_desc = tt::tt_metal::MetalContext::instance().get_cluster().get_soc_desc(0);
    DeviceAddr dram_bank_size = config_->dram_bank_size -
                                (config_->dram_unreserved_base - soc_desc.dram_view_address_offsets[0]) -
    ...
    for (uint32_t channel_id = 0; channel_id < config_->num_dram_channels; channel_id++) {
        bank_offsets.at(channel_id) = static_cast<uint64_t>(config_->dram_bank_offsets.at(channel_id));
    }
...
uint64_t Allocator::get_bank_offset(BufferType buffer_type, uint32_t bank_id) const {
    std::lock_guard<std::mutex> lock(mutex_);
    switch (buffer_type) {
