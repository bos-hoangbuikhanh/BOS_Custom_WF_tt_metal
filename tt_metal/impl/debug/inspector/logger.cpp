...
Logger::Logger(const std::filesystem::path& logging_path) : initialized(false), logging_path(logging_path) {
...
void Logger::log_program_kernel_compile_finished(
    const ProgramData& program_data, const KernelData& kernel_data) noexcept {
    if (!initialized) {
        return;
    }
    try {
        ...
        programs_ostream << "    duration_ns: "
                         << std::chrono::duration_cast<std::chrono::nanoseconds>(
                                timestamp - program_data.compile_started_timestamp)
                                .count()
                         << "\n";
...
void Logger::log_program_compile_finished(const ProgramData& program_data) noexcept {
    ...
    try {
        ...
        programs_ostream << "    duration_ns: "
                         << std::chrono::duration_cast<std::chrono::nanoseconds>(
                                program_data.compile_finished_timestamp - program_data.compile_started_timestamp)
                                .count()
                         << "\n";
        programs_ostream.flush();
    } catch (const std::exception& e) {
...
void Logger::log_program_binary_status_change(
    ...
    try {
        ...
        switch (status) {
            case ProgramBinaryStatus::NotSent: programs_ostream << "NotSent" << "\n"; break;
            case ProgramBinaryStatus::InFlight: programs_ostream << "InFlight" << "\n"; break;
            case ProgramBinaryStatus::Committed: programs_ostream << "Committed" << "\n"; break;
        }
        programs_ostream.flush();
    } catch (const std::exception& e) {
...
void Logger::log_mesh_workload_add_program(
...
void Logger::log_mesh_workload_set_program_binary_status(
    ...
    try {
        ...
        switch (status) {
            case ProgramBinaryStatus::NotSent: mesh_workloads_ostream << "NotSent" << "\n"; break;
            case ProgramBinaryStatus::InFlight: mesh_workloads_ostream << "InFlight" << "\n"; break;
            case ProgramBinaryStatus::Committed: mesh_workloads_ostream << "Committed" << "\n"; break;
        }
        mesh_workloads_ostream << "    timestamp_ns: " << convert_timestamp(std::chrono::high_resolution_clock::now())
                               << "\n";
        mesh_workloads_ostream.flush();
