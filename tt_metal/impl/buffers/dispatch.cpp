...
struct BufferWriteDispatchParams {
    ...
    DeviceAddr address = 0;  // [BOS_FIX] Changed from uint32_t to DeviceAddr (uint64_t) to support DRAM bank offsets
...
void populate_sharded_buffer_write_dispatch_cmds(
    ...
    CoreCoord target_dram_core;
    if (virtual_core.y == 3) {
        uint32_t dram_x = (virtual_core.x == 1 || virtual_core.x == 3 || virtual_core.x == 4) ? 4 : 0;
        target_dram_core = CoreCoord(dram_x, virtual_core.y);
    } else {
        target_dram_core = virtual_core;
    }

    command_sequence.add_dispatch_write_linear(
        0,
        buffer.device()->get_noc_unicast_encoding(k_dispatch_downstream_noc, target_dram_core),
        dispatch_params.address,
        data_size_bytes);
