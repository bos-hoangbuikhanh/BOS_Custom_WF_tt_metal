...
MetalContext::MetalContext() {
    ...
    for (ChipId device_id : cluster_->all_chip_ids()) {
        // [BOS-ARCH-001] 64-bit address
        dram_bank_offset_map_.emplace(device_id, std::vector<uint64_t>{});
...
CoreCoord MetalContext::virtual_noc0_coordinate(ChipId device_id, uint8_t noc_index, CoreCoord coord) {
    ...
    if (coord.x >= grid_size.x || coord.y >= grid_size.y) {
        ...
        log_debug(tt::LogMetal, "CoreCoord : {}", virtual_coord.str());
...
void MetalContext::generate_device_bank_to_noc_tables(ChipId device_id) {
    ...
    log_debug(tt::LogMetal, "allocator.num_dram_banks : {:x}", num_dram_banks);
    dram_bank_offset_map_[device_id].clear();
    dram_bank_offset_map_[device_id].resize(num_dram_banks);
    for (unsigned bank_id = 0; bank_id < num_dram_banks; bank_id++) {
        log_debug(
            tt::LogMetal, "allocator.get_bank_offset : {:x}", allocator.get_bank_offset(BufferType::DRAM, bank_id));
...
void MetalContext::initialize_device_bank_to_noc_tables(
    ...
    // [BOS-ARCH-001] 64-bit address
    const uint32_t dram_offset_sz_in_bytes = dram_bank_offset_map_[device_id].size() * sizeof(uint64_t);
...
dev_msgs::core_info_msg_t MetalContext::populate_core_info_msg(
    ...
    // set NoC2AXI core with noc0 NIU regs to opt-out multicast packets (ring0, y=3)
    // Ring0 X ID : Coordinate mapping
    uint32_t exclude_y = 1 << 3;
    cluster_->write_reg(&exclude_y, tt_cxy_pair(device_id, 0, 3), 0x110);
    cluster_->write_reg(&exclude_y, tt_cxy_pair(device_id, 4, 3), 0x110);
    cluster_->write_reg(&exclude_y, tt_cxy_pair(device_id, 5, 3), 0x110);

    // set NoC2AXI core with noc1 NIU regs to opt-out multicast packets (ring1, y=1)
    exclude_y = 1 << 1;
    cluster_->write_reg(&exclude_y, tt_cxy_pair(device_id, 1, 3), 0x110);
    cluster_->write_reg(&exclude_y, tt_cxy_pair(device_id, 2, 3), 0x110);
    cluster_->write_reg(&exclude_y, tt_cxy_pair(device_id, 3, 3), 0x110);

    // Determine which noc-coords are harvested
    std::vector<uint32_t> harvested_axis_coord;
    CoreCoord logical_grid_size = cluster_->get_soc_desc(device_id).get_grid_size(CoreType::TENSIX);
    // [BOS-ARCH-008] Not supported Havesting
...
void MetalContext::initialize_and_launch_firmware(ChipId device_id) {
    ...
    // [BOS-ARCH-017] Not supported barrier
    // cluster_->l1_barrier(device_id);
