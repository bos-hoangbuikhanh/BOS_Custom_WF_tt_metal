...
void DeviceCommand<hugepage_write>::add_prefetch_relay_linear(uint32_t noc_xy_addr, DeviceAddr lengthB, uint64_t addr) {
    ...
    auto remap_dram_xy_addr = [](uint32_t xy_addr) {
        uint32_t y = xy_addr >> 6;
        uint32_t x = xy_addr & 0x3F;
        if (y != 3) {
            return xy_addr;
        }
        // dram mapping
        uint32_t dram_x = (x == 1 || x == 3 || x == 4) ? 4 : 0;
        return (y << 6) | dram_x;
    };
    uint32_t remapped_noc_xy_addr = remap_dram_xy_addr(noc_xy_addr);
    auto initialize_relay_linear_cmd = [&](CQPrefetchCmdLarge* relay_linear_cmd) {
        ...
        relay_linear_cmd->relay_linear.noc_xy_addr = remapped_noc_xy_addr;
