...
constexpr uint64_t pcie_noc_xy =
    NOC_XY_PCIE_ENCODING(DYNAMIC_NOC_X(NOC_INDEX, PCIE_NOC_X), DYNAMIC_NOC_Y(NOC_INDEX, PCIE_NOC_Y));
...
void process_write_linear(uint32_t num_mcast_dests) {
    ...
    if (multicast) {
        ...
        num_mcast_dests += mcast_extra_ack_count(dst_noc);
        DPRINT << num_mcast_dests << " way multicast write " << num_mcast_dests << ENDL();
...
void process_write_packed(uint32_t flags, uint32_t* l1_cache) {
    ...
    while (count != 0) {
        ...
        // [BOS-ARCH-005] Support to count extra ack count
        if (mcast) {
            num_dests += mcast_extra_ack_count(dst_noc);
        }
        sub_cmd_ptr++;
...
void process_write_packed_large(uint32_t* l1_cache) {
    ...
    while (count != 0) {
        ...
        // [BOS-ARCH-005] Support to count extra ack count
        if (num_dests > 1) {
            num_dests += mcast_extra_ack_count(sub_cmd_ptr->noc_xy_addr);
        }
        auto wait_for_barrier = [&]() {
            if (!must_barrier) {
                return;
...
void process_go_signal_mcast_cmd() {
    ...
    if (multicast_go_offset != CQ_DISPATCH_CMD_GO_NO_MULTICAST_OFFSET) {
        ...
        // [BOS-ARCH-005] Support to count extra ack count
        uint32_t num_dests =
            num_worker_cores_to_mcast + mcast_extra_ack_count(dst_noc_addr_multicast >> NOC_ADDR_COORD_SHIFT);
    ...
    DPRINT << "process_go_signal_mcast_cmd" << ENDL();
