...
void FDMeshCommandQueue::enqueue_read_shard_from_core(
    ...
    if (address.virtual_core_coord.x == 5 && address.virtual_core_coord.y == 3) {
        // avoid (5, 3) core, it changes (5, 3) -> (0, 3)
        CoreCoord hacked_virtual_core_coord{0, 3};
        address.virtual_core_coord = hacked_virtual_core_coord;
    }
    if (size_bytes > 0) {
        this->reset_prefetcher_cache_manager();
