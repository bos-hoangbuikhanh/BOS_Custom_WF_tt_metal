...
protected:
    ...
    static FabricNodeId make_node(uint32_t mesh_id, uint32_t chip_id) { return FabricNodeId(MeshId{mesh_id}, chip_id); }
    ...
    static void verify_bidirectional_consistency(const TopologyMappingResult& result) {
        ...
        for (const auto& [asic, node] : result.asic_to_fabric_node) {
            ASSERT_TRUE(result.fabric_node_to_asic.count(node) > 0) << "Node not found in forward mapping";
