...
TEST(MeshContainerIteratorTraitsTest, STLAlgorithmsCompatibility) {
    ...
    // Test std::find_if
    auto it = std::find_if(container.begin(), container.end(), [](const auto& proxy) { return proxy.value() == 3; });
    ASSERT_NE(it, container.end());
    EXPECT_EQ(it->value(), 3);
    EXPECT_EQ(it->coord(), MeshCoordinate(1, 0));

    // Test std::count_if
    auto count = std::count_if(container.begin(), container.end(), [](const auto& proxy) { return proxy.value() > 2; });
    EXPECT_EQ(count, 3);

    // Test std::any_of
    bool has_value_4 =
        std::any_of(container.begin(), container.end(), [](const auto& proxy) { return proxy.value() == 4; });
    EXPECT_TRUE(has_value_4);

    // Test std::all_of
    bool all_non_negative =
        std::all_of(container.begin(), container.end(), [](const auto& proxy) { return proxy.value() >= 0; });
    EXPECT_TRUE(all_non_negative);

    // Test std::none_of
    bool none_greater_than_10 =
        std::none_of(container.begin(), container.end(), [](const auto& proxy) { return proxy.value() > 10; });
