...
protected:
    void SetUp() override {
        ...
        //     log_info(tt::LogTest, "Skipping test: Pre-conditions not satisfied");
        GTEST_SKIP();
        // }
...
private:
    ...
    bool are_env_vars_set() {
        //[BOS-ARCH-022] root dir is always set via env_set; kernel dir must still exist.
        bool are_set = false;
        ...
        //[BOS-ARCH-022] root dir is always set via env_set; kernel dir must still exist.
        if (tt::tt_metal::MetalContext::instance().rtoptions().get_kernel_dir().empty()) {
            log_info(tt::LogTest, "Skipping test: TT_METAL_KERNEL_PATH must be set");
            are_set = false;
        }
        return are_set;
