...
bool single_core_binary(
    ...
    // DO NOT use math fidelity masking (bit_cast with srca_fid_mask/srcb_fid_mask) for mul operation!
    std::transform(
        input0.begin(),
        input0.end(),
        input1.begin(),
    ...
    // [BOS-ARCH-021] For test success, return type must be std::vector<float>, not std::vector<bfloat16>!
    ...
    auto unpacked_dest_buffer_data = unpack_vector<bfloat16, uint32_t>(dest_buffer_data);
    ...
    pass &= is_close_vectors(dest_buffer_data_float, golden, std::function<bool(float, float)>([&](float a, float b) {
