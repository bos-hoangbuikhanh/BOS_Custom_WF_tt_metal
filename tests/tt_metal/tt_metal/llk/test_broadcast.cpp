...
// [BOS-ARCH-021] For test success,
// return type must be std::vector<float>, not std::vector<bfloat16>!
std::vector<float> gold_broadcast(
    ...
    // [BOS-ARCH-021] For test success,
    // return type must be std::vector<float>, not std::vector<bfloat16>!
    std::vector<float> golden(num_cols * num_rows);
    ...
    for (int i = 0; i < num_rows; i++) {
        for (int j = 0; j < num_cols; j++) {
            ...
            switch (op) {
                // [BOS-ARCH-021] For test success
                // DO NOT use math fidelity masking for broadcast operations!
                case EltwiseOp::ADD: {
                    golden[(i * num_cols) + j] =
...
void run_single_core_broadcast(
    ...
    std::vector<float> golden = gold_broadcast(
        input0,
        input1,
    ...
    // [BOS-ARCH-021] For test success
    // DO NOT use packed_golden! Use float vector comparison instead.
    ...
    // [BOS-ARCH-021] For test success
    // comparison must use is_close_vectors() with float vectors, not is_close_packed_vectors()!
    auto unpacked_dest_buffer_data = unpack_vector<bfloat16, uint32_t>(dest_buffer_data_untilized);
    std::vector<float> dest_buffer_data_float(unpacked_dest_buffer_data.size());
    std::transform(
        unpacked_dest_buffer_data.begin(),
        unpacked_dest_buffer_data.end(),
        dest_buffer_data_float.begin(),
        [](const bfloat16& val) { return static_cast<float>(val); });
    bool result = tt::test_utils::is_close_vectors(
        dest_buffer_data_float, golden, std::function<bool(float, float)>([&](float a, float b) {
            return tt::test_utils::is_close(a, b, 0.0155f);
        }));
    ASSERT_TRUE(result);
}
}  // namespace unit_tests::compute::broadcast
