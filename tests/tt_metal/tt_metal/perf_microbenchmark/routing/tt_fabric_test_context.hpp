...
public:
    ...
    void process_telemetry_data(TestConfig& built_test_config) {
        // Skip telemetry readback in latency test mode because we don't actually care about the values of the
        // telemetry. We only enable it so that latency tests take into account the overheads of having telemetry
        // enabled
...
private:
    ...
    std::vector<GoldenLatencyEntry>::iterator fetch_corresponding_golden_latency_entry(
