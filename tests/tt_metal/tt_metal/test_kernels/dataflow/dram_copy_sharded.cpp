...
void kernel_main() {
    ...
    uint64_t src_base_addr =
        noc_async_read_tile_dram_sharded_set_state<true>(dram_buffer_src_addr, page_size, bank_id, vc);
    DPRINT << " src_base_addr : " << HEX() << src_base_addr << ENDL();
    uint32_t num_pages = dram_buffer_size / page_size;
    uint32_t curr_block_trid = 1;
    uint32_t l1_read_addr = 0;
    for (uint32_t block = 0; block < num_blocks; ++block) {
        DPRINT << "BLOCK " << block << ENDL();
        uint32_t l1_write_addr = l1_buffer_addr;
        ...
        noc_async_read_barrier_with_trid(curr_block_trid);
    }

    noc_async_read_barrier_with_trid(curr_block_trid);
    DPRINT << l1_buffer_addr << "\n";
    DPRINT << *(std::uint32_t*)(l1_buffer_addr) << "\n";
