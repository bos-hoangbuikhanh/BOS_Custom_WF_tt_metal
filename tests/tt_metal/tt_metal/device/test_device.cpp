...
// [BOS-ARCH-020] For getting DRAM offset from SoC description
#include "tt_cluster.hpp"
...
TEST_F(MeshDeviceFixture, PingAllLegalDramChannels) {
    for (unsigned int id = 0; id < num_devices_; id++) {
        {
            const auto& soc_d = tt::tt_metal::MetalContext::instance().get_cluster().get_soc_desc(id);
            size_t start_byte_address = devices_.at(id)->allocator()->get_base_allocator_addr(HalMemType::DRAM) -
                                        soc_d.dram_view_address_offsets[0];
            std::cout << "start_byte_address: " << start_byte_address << std::endl;
...
TEST_F(MeshDeviceFixture, TensixValidateKernelDoesNotTargetHarvestedCores) {
    for (unsigned int id = 0; id < num_devices_; id++) {
        ...
        log_info(tt::LogTest, "num_l1_banks: ={}", num_l1_banks);
        for (uint32_t bank_id = 0; bank_id < num_l1_banks; bank_id++) {
            CoreCoord logical_core = mesh_device->allocator()->get_logical_core_from_bank_id(bank_id);
            ...
            // std::cout << "Logical core " + logical_core.str() + " should have " +
            //                                                    std::to_string(expected_value) + " but got " +
            //                                                    std::to_string(output.at(0)) << std::endl;
...
TEST_F(MeshDeviceFixture, TensixTestL1ToPCIeAt16BAlignedAddress) {
    ...
    // std::vector<uint32_t> src = generate_uniform_random_vector<uint32_t>(0, UINT32_MAX, size_bytes /
    // sizeof(uint32_t)); EXPECT_EQ(MetalContext::instance().hal().get_alignment(HalMemType::L1), 16); uint32_t
    // num_16b_writes = size_bytes / MetalContext::instance().hal().get_alignment(HalMemType::L1);
