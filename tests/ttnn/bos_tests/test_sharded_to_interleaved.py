...
def test_sharded_rm(
    ...
    known_issue_cases = [
        # Case 1: ([1, 1, 100, 80], BLOCK_SHARDED, (32, 32), [[(0, 0), (3, 3)]])
        (
            input_shape == [1, 1, 100, 80]
            and shard_scheme == ttnn.TensorMemoryLayout.BLOCK_SHARDED
            and shard_shape == (32, 32)
            and core_range_set == [[(0, 0), (3, 3)]]
        ),
        # Case 2: ([1, 1, 64, 80], BLOCK_SHARDED, (32, 32), [[(0, 0), (3, 1)]])
        (
            input_shape == [1, 1, 64, 80]
            and shard_scheme == ttnn.TensorMemoryLayout.BLOCK_SHARDED
            and shard_shape == (32, 32)
            and core_range_set == [[(0, 0), (3, 1)]]
        ),
    ]

    if any(known_issue_cases):
        known_issue_msg = f"KNOWNISSUE: Test case skipped due to known issue - input_shape={input_shape}, shard_scheme={shard_scheme}, shard_shape={shard_shape}, core_range_set={core_range_set}"
        print(known_issue_msg)
        pytest.skip(known_issue_msg)
