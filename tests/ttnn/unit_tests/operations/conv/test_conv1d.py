...
# --- bos-metal conv1d ---
import math
from bos_metal import op, helpers, engine
def run_conv1d_test(
    device,
    B,
    C_in,
    ...
    """Helper function to run a single conv1d test"""
    # Create PyTorch model
    torch.manual_seed(42)
    x_torch = torch.randn(B, C_in, L_in)
    x_tt = ttnn.from_torch(
        x_torch.transpose(-1, -2),
        device=device,
        dtype=ttnn.bfloat16,
    ...
    # Process state dict
    st = engine.process_model_state_dict(conv1d_torch, x_torch)

    # Create TT-Metal model
    conv1d_tt = op.Conv1d(
        in_channels=C_in,
        out_channels=C_out,
        kernel_size=kernel_size,
    ...
    conv1d_tt.config.deallocate_activation = True
    conv1d_tt.load_state_dict(st)

    out_tt = ttnn.to_torch(conv1d_tt(x_tt)).squeeze(1)
    out_tt = out_tt.reshape(B, -1, C_out)
    out_torch = conv1d_torch(x_torch).transpose(-1, -2)
    
    # Compare results
    passing, msg = helpers.compare_tensors(out_tt, out_torch, pcc=pcc)
    assert passing, f"Conv1d test failed! PCC: {msg}"

@pytest.mark.parametrize("device_params", [{"l1_small_size": 16384}], indirect=True)
@pytest.mark.parametrize("B", [1])
@pytest.mark.parametrize("C_in", [16, 31, 100, 256])
@pytest.mark.parametrize("C_out", [101, 255])
@pytest.mark.parametrize("L_in", [128, 4100])
@pytest.mark.parametrize("kernel_size", [1, 3, 7])
@pytest.mark.parametrize("stride", [1])
@pytest.mark.parametrize("padding", ["same", 1, 2, 3])
@pytest.mark.parametrize("dilation", [1, 2])
@pytest.mark.parametrize("dw", [True, False])
def test_conv1d_bos_metal(device, B, C_in, C_out, L_in, kernel_size, stride, padding, dilation, dw):
    if dw:
        groups = C_in
        C_out = C_in
    else:
    ...
    run_conv1d_test(device, B, C_in, C_out, L_in, kernel_size, stride, padding, dilation, groups)


@pytest.mark.parametrize("device_params", [{"l1_small_size": 16384}], indirect=True)
@pytest.mark.parametrize("B", [1])
@pytest.mark.parametrize("C_in", [512])
@pytest.mark.parametrize("C_out", [511, 512])
@pytest.mark.parametrize("L_in", [4100, 8192])
@pytest.mark.parametrize("kernel_size", [3, 7])
@pytest.mark.parametrize("stride", [1])
@pytest.mark.parametrize("padding", ["same", 1, 2, 3])
@pytest.mark.parametrize("dilation", [1, 2])
@pytest.mark.parametrize("dw", [True, False])
def test_conv1d_bos_metal_large(device, B, C_in, C_out, L_in, kernel_size, padding, stride, dilation, dw):
    if dw:
        groups = C_in
        C_out = C_in
    else:
    ...
    if padding == "same":
        padding = dilation * (kernel_size - 1) // 2
    run_conv1d_test(device, B, C_in, C_out, L_in, kernel_size, stride, padding, dilation, groups, slice_config=op.Conv2dSliceConfig(num_slices=4))

if __name__ == "__main__":
    device = ttnn.open_device(device_id=0, l1_small_size=16384)
    test_conv1d_bos_metal_large(device, B=1, C_in=512, C_out=512, L_in=4100, kernel_size=7, stride=1, padding="same", dilation=1, dw=True)
