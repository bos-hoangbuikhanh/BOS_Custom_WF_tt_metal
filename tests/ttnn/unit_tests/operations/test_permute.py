...
@pytest.mark.parametrize("h", [159])
@pytest.mark.parametrize("w", [159])
@pytest.mark.parametrize(
    "dtype",
    [
        ttnn.bfloat16,
...
def test_permute_oft(device, h, w, dtype):
    torch.manual_seed(2005)
    shape = (1, 3, h, w)
    torch_input_tensor = random_torch_tensor(dtype, shape)
    torch_output_tensor = torch.permute(torch_input_tensor, (0, 2, 3, 1))

    input_tensor = ttnn.from_torch(torch_input_tensor)
    input_tensor = ttnn.to_device(input_tensor, device, memory_config=ttnn.DRAM_MEMORY_CONFIG)
    output_tensor = ttnn.permute(input_tensor, (0, 2, 3, 1))
    output_tensor = ttnn.to_layout(output_tensor, ttnn.ROW_MAJOR_LAYOUT)
    output_tensor = ttnn.from_device(output_tensor)
    output_tensor = ttnn.to_torch(output_tensor)
