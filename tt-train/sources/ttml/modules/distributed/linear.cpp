...
autograd::TensorPtr RowParallelLinear::operator()(const autograd::TensorPtr& tensor) {
    ...
    /*
        All reduce with noop backward to avoid double all reduce in backward pass. This happens due to broadcast (no op
       in forward pass) does all reduce in backward pass. See similar implementation in fairscale for more details.
