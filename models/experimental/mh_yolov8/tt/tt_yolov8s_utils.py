...
def ttnn_decode_bboxes(device, distance, anchor_points, xywh=True, dim=1, save_pt=False, regen_input=False):
    lt, rb = ttnn.split(distance, 2, 1, memory_config=ttnn.L1_MEMORY_CONFIG)  # if done in tile : tt-metal issue #17017 >= v0.57.0-rc36
    # lt = ttnn.slice(distance, [0, 0, 0], [1, 2, distance.shape[2]], memory_config=ttnn.L1_MEMORY_CONFIG)
    # rb = ttnn.slice(distance, [0, 2, 0], [1, 4, distance.shape[2]], memory_config=ttnn.L1_MEMORY_CONFIG)
    # lt = ttnn.to_layout(lt, ttnn.TILE_LAYOUT)
    # rb = ttnn.to_layout(rb, ttnn.TILE_LAYOUT)
    if save_pt:
        torch_lt = torch.Tensor(ttnn.to_torch(ttnn.from_device(lt))).to(torch.float32)
        torch.save(torch.Tensor(torch_lt), "ttnn_detect_22._inference.dbox.lt.pt")
        torch_rb = torch.Tensor(ttnn.to_torch(ttnn.from_device(rb))).to(torch.float32)
        torch.save(torch.Tensor(torch_rb), "ttnn_detect_22._inference.dbox.rb.pt")

    x1y1 = anchor_points - lt
    x2y2 = anchor_points + rb
    if save_pt:
        torch_x1y1 = torch.Tensor(ttnn.to_torch(ttnn.from_device(x1y1))).to(torch.float32)
        torch.save(torch.Tensor(torch_x1y1), "ttnn_detect_22._inference.dbox.x1y1.pt")
        torch_x2y2 = torch.Tensor(ttnn.to_torch(ttnn.from_device(x2y2))).to(torch.float32)
        torch.save(torch.Tensor(torch_x2y2), "ttnn_detect_22._inference.dbox.x2y2.pt")
    if xywh:
        c_xy = x1y1 + x2y2
        c_xy = ttnn.div(c_xy, 2, dtype=ttnn.bfloat8_b)
        _ = ttnn.to_layout(c_xy, ttnn.ROW_MAJOR_LAYOUT)
        c_xy = ttnn.to_layout(c_xy, ttnn.ROW_MAJOR_LAYOUT)
        if save_pt:
            torch_c_xy = torch.Tensor(ttnn.to_torch(ttnn.from_device(c_xy))).to(torch.float32)
            torch.save(torch.Tensor(torch_c_xy), "ttnn_detect_22._inference.dbox.c_xy.pt")
        wh = ttnn.subtract(x2y2, x1y1, dtype=ttnn.bfloat8_b)
        wh = ttnn.to_layout(wh, ttnn.ROW_MAJOR_LAYOUT)
        if save_pt:
            torch_wh = torch.Tensor(ttnn.to_torch(ttnn.from_device(wh))).to(torch.float32)
            torch.save(torch.Tensor(torch_wh), "ttnn_detect_22._inference.dbox.wh.pt")
