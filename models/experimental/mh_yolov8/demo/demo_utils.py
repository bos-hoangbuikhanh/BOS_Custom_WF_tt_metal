...
def letterbox_transform(im, res):
    return [LetterBox(img=x, new_shape=res) for x in im]


def Resize(img, new_shape=(320, 320), auto=False, scaleFill=False, scaleup=True, center=True, stride=32):
    shape = img.shape[:2]
    if isinstance(new_shape, int):
        new_shape = (new_shape, new_shape)
...
def resize_transform(im, res):
    return [Resize(img=x, new_shape=res) for x in im]
...
def preprocess(im, res):
    ...
    if not_tensor:
        # im = np.stack(letterbox_transform(im, res))
        im = np.stack(resize_transform(im, res))
...
def scale_boxes_letterbox(img1_shape, boxes, img0_shape, ratio_pad=None, padding=True, xywh=False):
...
def scale_boxes(img1_shape, boxes, img0_shape, ratio_pad=None, padding=True, xywh=False):
    x_gain = img1_shape[1] / img0_shape[1]
    y_gain = img1_shape[0] / img0_shape[0]
    boxes[..., 0] /= x_gain
    boxes[..., 1] /= y_gain
    boxes[..., 2] /= x_gain
    boxes[..., 3] /= y_gain
...
def postprocess(preds, img, orig_imgs, batch, names):
    args = {"conf": 0.25, "iou": 0.45, "agnostic_nms": False, "max_det": 300, "classes": None}
    ...
    for pred, orig_img, img_path in zip(preds, orig_imgs, batch[0]):
        # pred[:, :4] = scale_boxes_letterbox(img.shape[2:], pred[:, :4], orig_img.shape)
