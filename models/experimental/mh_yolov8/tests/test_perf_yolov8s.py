...
def test_yolov8s(device, input_tensor, use_weights_from_ultralytics):
    ...
    # input_tensor = torch.nn.functional.pad(input_tensor, (0, 0, 0, 0, 0, 13, 0, 0), value=0)
    ...
    if c < min_channels:
        ...
        input_tensor = torch.nn.functional.pad(input_tensor, (0, channel_padding_needed, 0, 0, 0, 0), value=0.0)
        c = min_channels
    input_tensor = input_tensor.reshape(1, 1, n * h * w, c)
    ...
    _ = [ttnn.deallocate(x) for x in ttnn_output_tensor[1]]
