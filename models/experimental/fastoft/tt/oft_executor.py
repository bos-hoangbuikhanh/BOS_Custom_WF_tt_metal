...
"""
OFT-specific executor implementations with preprocessing/run pattern support.

These executors are adapted from the base tt_cnn executors to support models
that require separate preprocess() and run() methods.
"""
...
class ModelExecutor(Executor):
    ...
    def _execute_single(self, input_tensor):
        l1_input_tensor = ttnn.to_device(input_tensor, device=self.device)
        preprocessed = self.model.preprocess(l1_input_tensor)
        self._deallocate_structured_tensor(l1_input_tensor, force=True)

        output_tensor = self.model.run(preprocessed)

        self._deallocate_structured_tensor(preprocessed, force=True)
...
class TracedModelExecutor(Executor):
    ...
    def _run_model_for_compilation(self, host_input):
        ...
        self._compilation_output_tensor = self.model.run(l1_input_for_compile)
        ttnn.deallocate(l1_input_for_compile, force=True)
    ...
    def _capture_execution_trace(self, host_input):
        ...
        self.output_tensor = self.model.run(l1_input_for_trace)
    ...
    def _prepare_l1_input(self, host_input):
        ...
        return self.model.preprocess(self.dram_input_tensor)
    ...
    def _execute_single(self, input_tensor):
        ...
        # For OFT: DRAM input is interleaved, L1 is also interleaved (None)
        # Use DRAM tensor directly (HACK)
        if self.l1_input_tensor.is_allocated():
            ttnn.deallocate(self.l1_input_tensor, force=True)
        self.l1_input_tensor = self.model.preprocess(self.dram_input_tensor)
...
class MultiCQTracedModelOverlappedInputExecutor(Executor):
    ...
    def _compile_model(self, host_input):
        ...
        # Convert to L1 and run model
        ttnn.wait_for_event(self.CQ_OPS_AND_OUTPUT_READ, write_event)
        l1_input_tensor = self.model.preprocess(self.dram_input_tensor)
        self.op_event = ttnn.record_event(self.device, self.CQ_OPS_AND_OUTPUT_READ)

        self._compilation_output_tensor = self.model.run(l1_input_tensor)
    ...
    def _capture_trace(self, host_input):
        ...
        self.output_tensor = self.model.run(l1_input_tensor)
        if l1_input_tensor.is_allocated():
            ttnn.deallocate(l1_input_tensor, force=True)
...
class MultiCQModelOverlappedDramInputExecutor(Executor):
    """
    Multi-command queue model executor that overlaps input transfers with model execution
    for improved throughput without using tracing.
    ...
    This variant uses DRAM input directly without L1 conversion.
    """
    ...
    def _compile_model(self, host_input):
        ...
        ttnn.wait_for_event(self.CQ_OPS_AND_OUTPUT_READ, write_event)
        processed_tensor = self.model.preprocess(self.dram_input_tensor)
        output_tensor = self.model.run(processed_tensor)
        self.op_event = ttnn.record_event(self.device, self.CQ_OPS_AND_OUTPUT_READ)
