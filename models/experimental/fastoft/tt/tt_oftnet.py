...
class TTOftNet:
    def __init__(
        ...
        self.lat8 = TtConv2d(layer_args.lat8["optimized_configuration"], device) if (oft_mode == OftMode.ORIGINAL or oft_mode == OftMode.OFT8) else None

        self.lat16 = TtConv2d(layer_args.lat16["optimized_configuration"], device) if (oft_mode == OftMode.ORIGINAL or oft_mode == OftMode.OFT16) else None

        self.lat32 = TtConv2d(layer_args.lat32["optimized_configuration"], device) if (oft_mode == OftMode.ORIGINAL or oft_mode == OftMode.OFT32) else None
        ...
        ) if (oft_mode == OftMode.ORIGINAL or oft_mode == OftMode.OFT8) else None
        self.oft16 = TtOFT(
            device,
            state_dict.oft16,
            input_channels,
            ...
            align_corners=True,
            return_intermediates=return_intermediates
        ) if (oft_mode == OftMode.ORIGINAL or oft_mode == OftMode.OFT16) else None
        self.oft32 = TtOFT(
            device,
            state_dict.oft32,
            input_channels,
            ...
            align_corners=True,
            return_intermediates=return_intermediates
        ) if (oft_mode == OftMode.ORIGINAL or oft_mode == OftMode.OFT32) else None
    ...
    def forward_oft(self, device, lat8, lat16, lat32, calib, grid):
        ...
        (
            ...
            bbox_top_right8, 
            bbox_btm_left8,
         ) = self.oft8.forward(device, lat8, calib, grid)  if self.oft8 is not None else (None, None, None, None, None, None)
        ...
        (
            ...
            bbox_top_right16,
            bbox_btm_left16,
        ) = self.oft16.forward(device, lat16, calib, grid) if self.oft16 is not None else (None, None, None, None, None, None)
        ...
        (
            ...
            bbox_top_right32,
            bbox_btm_left32,
        ) = self.oft32.forward(device, lat32, calib, grid) if self.oft32 is not None else (None, None, None, None, None, None)
