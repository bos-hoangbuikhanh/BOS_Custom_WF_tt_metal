...
def calculate_initialization_parameters(
    ...
    area = (
        (bbox_corners[..., 2:] - bbox_corners[..., :2]).prod(dim=-1) * feature_height * feature_width * 0.25 + torch.tensor(EPSILON, dtype=bbox_corners.dtype)
    ).unsqueeze(1)
   
    visible = area > torch.tensor(EPSILON, dtype=area.dtype)
    # set visible to False for all bboxes that are out of bounds
    area = 1 / area
    area_nhwc = torch.broadcast_to(
        area.permute(0, 2, 1, 3),
        (1, depth * width, channels, height)).reshape(1, depth * width, 1, channels * height).permute(0, 2, 1, 3)
    visible_nhwc = torch.broadcast_to(
        visible.permute(0, 2, 1, 3),
        (1, depth * width, channels, height)).reshape(1, depth * width, 1, channels * height).permute(0, 2, 1, 3)
    ...
    top_left_bc_tt = prepare_ttnn_grid(top_left_bc, device, use_precomputed_grid, ttnn.float32, [batch, feature_height, feature_width, channels], height, mode=mode, align_corners=align_corners)
    top_left_bc_tt = ttnn.reshape(top_left_bc_tt, [batch_size, 1, top_left_bc_tt.shape[1], top_left_bc_tt.shape[3]])

    btm_right_bc_tt = prepare_ttnn_grid(btm_right_bc, device, use_precomputed_grid, ttnn.float32, [batch, feature_height, feature_width, channels], height, mode=mode, align_corners=align_corners)
    btm_right_bc_tt = ttnn.reshape(btm_right_bc_tt, [batch_size, 1, btm_right_bc_tt.shape[1], btm_right_bc_tt.shape[3]])

    top_right_bc_tt = prepare_ttnn_grid(top_right_bc, device, use_precomputed_grid, ttnn.float32, [batch, feature_height, feature_width, channels], height, mode=mode, align_corners=align_corners)
    top_right_bc_tt = ttnn.reshape(top_right_bc_tt, [batch_size, 1, top_right_bc_tt.shape[1], top_right_bc_tt.shape[3]])

    btm_left_bc_tt = prepare_ttnn_grid(btm_left_bc, device, use_precomputed_grid, ttnn.float32, [batch, feature_height, feature_width, channels], height, mode=mode, align_corners=align_corners)
...
class OFT:
    ...
    def forward(self, device, features, calib, grid):
        ...
        for i in range(self.num_slices):
            ...
            top_left_grid = ttnn.to_memory_config(
                self.bbox_corners[0][i], grid_memory_config
            )
            ...
            top_right_grid = ttnn.to_memory_config(
                self.bbox_corners[2][i], grid_memory_config
            )
            ...
            btm_right_grid = ttnn.to_memory_config(
                self.bbox_corners[1][i], grid_memory_config
            )
            ...
            btm_left_grid = ttnn.to_memory_config(
                self.bbox_corners[3][i], grid_memory_config
            )
