...
class OFTPerformanceRunnerInfra:
    ...
    def setup_dram_sharded_input(self, device):
        ...
        # tt_input = torch.nn.functional.pad(self.input_tensor_host, (0, 0, 0, 0, 0, 8 - 3), mode="constant", value=0)
        # convert to channel last
        # torch_input = torch_input.permute(0, 2, 3, 1)

        n, h, w, c = tt_input.shape
        
        # Create HEIGHT_SHARDED L1 memory config (ResNet pattern)
        # For Blackhole 20 cores: use 5x4 grid = 20 cores
        core_grid = ttnn.CoreGrid(y=4, x=5)
        num_cores = core_grid.x * core_grid.y  # 20 cores
        
        # Calculate shard height: divide total elements by number of cores
        # After TILE layout, c=3 becomes c=32 (padded)
        c_padded = 32  # TILE layout pads 3 -> 32
        total_height = n * h * w  # 1 * 384 * 1280 = 491520
        shard_h = (total_height + num_cores - 1) // num_cores  # Ceiling division
        
        # Ensure shard_h is multiple of 32 for TILE layout
        shard_h = ((shard_h + 31) // 32) * 32
        
        # Create shard spec
        grid_coord = ttnn.CoreCoord(core_grid.x - 1, core_grid.y - 1)
        shard_grid = ttnn.CoreRangeSet({ttnn.CoreRange(ttnn.CoreCoord(0, 0), grid_coord)})
        shard_spec = ttnn.ShardSpec(shard_grid, (shard_h, c_padded), ttnn.ShardOrientation.ROW_MAJOR)
        
        # L1 sharded memory config
        ...
        logger.info(f"[Setup] Created L1 HEIGHT_SHARDED config: cores={num_cores}, shard_shape=({shard_h}, {c_padded})")
    ...
    def run(self, input_tensor):
        ...
        self.input_tensor = input_tensor
        (intermediates, layer_names), tt_scores, tt_pos_offsets, tt_dim_offsets, tt_ang_offsets = (
            self.tt_model.forward(self.device, self.input_tensor, self.tt_calib, self.tt_grid)
        )
    ...
    def validate(self, tt_outs_torch):
        ...
        return True, "Validation passed. {len(tt_objects_torch)} objects detected."
...
def _run_model_pipeline(
    ...
    """
    # Store input for preprocessing
    test_infra.input_tensor_host = tt_inputs
    ...
    for i, output in enumerate(outputs):
        ...
        passed, pcc_message = test_infra.validate(torch_outputs)
...
def test_perf_oft(
    ...
    if expected_inference_time is None:
        test_name = f"test_perf{'_trace' if 'trace' in model_version else ''}{'_2cqs' if '2cqs' in model_version else '_1cq'}"
    ...
    if input_image_path is None:
        input_image_path = os.path.abspath(
            os.path.join(os.path.dirname(__file__), "..", "resources", "000013.jpg")
        )
    if calib_path is None:
        calib_path = os.path.abspath(
            os.path.join(os.path.dirname(__file__), "..", "resources", "000013.txt")
        )
    ...
    # Load inputs and reference model
    ...
    with torch.no_grad():
        ...
        if "oft_trace_2cqs" in model_version:
            _run_model_pipeline(
                device, input_tensor, test_infra, num_warmup_iterations, num_measurement_iterations,
                num_command_queues=2, trace=True
            )
        elif "oft_trace" in model_version:
            _run_model_pipeline(
                device, input_tensor, test_infra, num_warmup_iterations, num_measurement_iterations,
                num_command_queues=1, trace=True
            )
        elif "oft_2cqs" in model_version:
            _run_model_pipeline(
                device, input_tensor, test_infra, num_warmup_iterations, num_measurement_iterations,
                num_command_queues=2, trace=False
            )
        elif "oft" in model_version:
            _run_model_pipeline(
                device, input_tensor, test_infra, num_warmup_iterations, num_measurement_iterations,
                num_command_queues=1, trace=False
            )
        else:
    ...
    # Assert inference time is within 5% of expected
    tolerance = 0.05
