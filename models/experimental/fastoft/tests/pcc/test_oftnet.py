...
import os
import math
from ...reference.oftnet import OftNet, OftMode, get_num_topdown_layers
from ...tt.model_configs import ModelOptimizations
from ...tt.tt_oftnet import TTOftNet
from ...tt.tt_resnet import TTBasicBlock
from ...tests.common import GRID_RES, GRID_SIZE, GRID_HEIGHT, Y_OFFSET, H_PADDED, W_PADDED
from ...tests.common import load_checkpoint
from ...reference.oftnet import FrontendMode

from ...reference.utils import make_grid, load_calib, load_image
from ...reference.utils import get_abs_and_relative_error

from tests.ttnn.utils_for_testing import check_with_pcc
from ...tt.model_preprocessing import create_OFT_model_parameters, fuse_conv_bn_parameters, fuse_imagenet_normalization
...
@pytest.mark.parametrize(
    "model_dtype, oft_mode, pcc_scores_oft, pcc_positions_oft, pcc_dimensions_oft, pcc_angles_oft",
    # fmt: off
    [
       ( torch.float32,     OftMode.OFT8, 0.980, 0.968, 0.999, 0.964),
    ],
    # fmt: on
    ids=[
        # "bfp16_original",
...
def test_oftnet(
    ...
    for i, (out, tt_out, layer_name) in enumerate(zip(intermediates, tt_intermediates, intermediates_names)):
        # convert tt output to torch, channel first, and correct shape
        if out is None or tt_out is None:
            logger.debug(f"Skipping PCC check for intermediate {i} {layer_name} due to None output")
            continue
    ...
    if SAVE_OUTPUTS:
        ...
        test_config = f"{oft_mode}_{model_dtype}_host_oft"
