...
import pytest
import math
from ...tt.tt_oft import OFT
from ...reference.oft import OFT as ReferenceOFT
from tests.ttnn.utils_for_testing import check_with_pcc
from ...tt.model_preprocessing import create_OFT_model_parameters_oft
from ...reference.utils import get_abs_and_relative_error, make_grid
...
@pytest.mark.parametrize(
    "input_shape, output_channels, cell_size, grid_height, scale, torch_model_dtype, use_precomputed_grid, pcc_integral_img, pcc_output, num_slices",
    [
        # fmt: off
        # feats8 {float32,bfloat16} x {use_precomputed_grid, no_use_precomputed_grid}
        ((1, 256, 48, 160), 128, 0.5, 4, 1 / 8, torch.float32,   True, 0.999, 0.992, 44),
        ((1, 256, 48, 160), 128, 0.5, 4, 1 / 8, torch.bfloat16,  True, 0.999, 0.373, 44),
        # # feats16 {float32,bfloat16} x {use_precomputed_grid, no_use_precomputed_grid}
        ((1, 256, 24, 80), 128, 0.5, 4, 1 / 16, torch.float32,   True, 0.999, 0.997, 44),
        ((1, 256, 24, 80), 128, 0.5, 4, 1 / 16, torch.bfloat16,  True, 0.999, 0.472, 44),
        # # feats32 {float32,bfloat16} x {use_precomputed_grid, no_use_precomputed_grid}
        ((1, 256, 12, 40), 128, 0.5, 4, 1 / 32, torch.float32,   True, 0.999, 0.999, 44),
        ((1, 256, 12, 40), 128, 0.5, 4, 1 / 32, torch.bfloat16,  True, 0.999, 0.636, 44),
        # fmt: on
    ],
    ids=[
...
def test_oft_forward(
    ...
    # Extract input channels from input shape
    input_channels = input_shape[1]  # 256 channels from input tensor
    ref_oft = ReferenceOFT(input_channels, output_channels, cell_size, grid_height, scale=scale, dtype=torch_model_dtype)
    ...
    tt_features = ttnn.from_torch(features_nhwc, dtype=ttnn.float32, layout=ttnn.TILE_LAYOUT, device=device)
