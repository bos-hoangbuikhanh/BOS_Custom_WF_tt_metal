...
class OFT(nn.Module):
    def __init__(self, input_channels, output_channels, cell_size, grid_height, scale, dtype):
        super().__init__()
        ...
        self.conv3d = nn.Linear((len(y_corners) - 1) * input_channels, output_channels, dtype=dtype)
        self.scale = scale
        self.dtype = dtype
    ...
    def forward(self, features, calib, grid):
        ...
        top_left = F.grid_sample(integral_img, bbox_corners[..., [0, 1]], align_corners=True, mode='nearest', padding_mode="zeros")
        btm_right = F.grid_sample(integral_img, bbox_corners[..., [2, 3]], align_corners=True, mode='nearest', padding_mode="zeros")
        top_right = F.grid_sample(integral_img, bbox_corners[..., [2, 1]], align_corners=True, mode='nearest', padding_mode="zeros")
        btm_left = F.grid_sample(integral_img, bbox_corners[..., [0, 3]], align_corners=True, mode='nearest', padding_mode="zeros")
