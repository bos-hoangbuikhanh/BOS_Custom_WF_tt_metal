...
from ..reference.oftnet import OftNet, OftMode, get_num_topdown_layers, FrontendMode
...
from ..reference.utils import print_object_comparison
...
from ..tt.model_preprocessing import create_OFT_model_parameters, create_decoder_model_parameters, fuse_conv_bn_parameters, fuse_imagenet_normalization
...
@pytest.mark.parametrize(
    ...
    # fmt: off
    [
    #  ( torch.float32, OftMode.ORIGINAL, FrontendMode.ORIGINAL, 0.972, 0.967, 0.960, 0.972), # for reference - tested with original weights
       ( torch.float32, OftMode.OFT8, FrontendMode.REDUCED, 0.959, 0.988, 0.999, 0.998),
    ],
    # fmt: on
)
@torch.no_grad()
def test_demo_inference(
    ...
    # Get tt_scores on host and save it
    # tt_scores_host = ttnn.to_torch(tt_scores)
    # torch.save(tt_scores_host, os.path.join(output_dir, f"tt_scores_{basename}.pt"))
    # logger.info(f"Saved tt_scores to {output_dir}/tt_scores_{basename}.pt")
