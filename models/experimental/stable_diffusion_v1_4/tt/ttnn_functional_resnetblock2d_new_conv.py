...
import ttnn
from ..sd_helper_funcs import reshard_for_output_channels_divisibility
...
config_override = {
    ...
    (1280, 2560, 16, 16): {"act_block_h": 32},
...
class resnetBlock2D:
    def __init__(
        ...
        self.norm1_weight_fallback = ttnn.to_torch(self.parameters.norm1.weight).float().reshape(-1)
        self.norm1_bias_fallback = ttnn.to_torch(self.parameters.norm1.bias).float().reshape(-1)

        self.norm2_weight_fallback = ttnn.to_torch(self.parameters.norm2.weight).float().reshape(-1)
        self.norm2_bias_fallback = ttnn.to_torch(self.parameters.norm2.bias).float().reshape(-1)
    ...
    def __call__(
        ...
        *,
        ...
        use_fallback = (self.conv2_input_height * self.conv2_input_width) == 4096 or (in_channels == 1920)

        if use_fallback:
            hidden_states_torch = ttnn.to_torch(hidden_states).float()
            hidden_states_torch = hidden_states_torch.squeeze(1).permute(0, 2, 1)

            out_torch = torch.nn.functional.group_norm(
                hidden_states_torch,
                num_groups=groups,
                weight=self.norm1_weight_fallback,
                bias=self.norm1_bias_fallback,
                eps=eps,
            )

            out_torch = out_torch.permute(0, 2, 1).unsqueeze(1).contiguous()
        ...
        hidden_states = ttnn.to_layout(
            hidden_states, ttnn.TILE_LAYOUT
        )  # Added bez of the error: Unary operation requires tensor to be in Tile layout when working with non-sharded input tensor. Input tensor layout: 0

        hidden_states = nonlinearity(hidden_states, memory_config=ttnn.get_memory_config(hidden_states))

        is_heavy_channel = self.conv1_in_channels in [960, 1280, 1920, 2560]
        seq_len = self.batch_size * self.conv2_input_height * self.conv2_input_width
        is_large_spatial = seq_len == 8192

        if is_heavy_channel or is_large_spatial:
            hidden_states_dram = ttnn.to_memory_config(hidden_states, ttnn.DRAM_MEMORY_CONFIG)
            ttnn.deallocate(hidden_states)
            hidden_states = hidden_states_dram

        if is_heavy_channel:
            shard_layout = None
            slice_config = None  # Enables DRAM Slicing
        else:
        ...
        if self.batch_size > 1:
            current_shape = hidden_states.shape

            seq_len_per_batch = current_shape[2] // self.batch_size

            if hidden_states.memory_config().is_sharded():
                hidden_states = ttnn.to_memory_config(hidden_states, ttnn.L1_MEMORY_CONFIG)

            hidden_states = ttnn.reshape(
                hidden_states, (self.batch_size, 1, seq_len_per_batch, self.conv1_out_channels)
            )
        ...
        #     dtype=ttnn.bfloat8_b,
        # ) # FIXME: Using fallback group_norm
        ...
        if use_fallback:
            ...
            out_torch = torch.nn.functional.group_norm(
                hidden_states_torch,
                num_groups=groups,
                weight=self.norm2_weight_fallback,
                bias=self.norm2_bias_fallback,
                eps=eps,
            )
        ...
        if self.conv1_in_channels == 960 or self.conv1_in_channels == 2560:
            shard_layout = None
        ...
        if use_in_shortcut:
            ...
            is_heavy_shortcut = self.conv_shortcut_in_channels >= 640

            if is_heavy_shortcut:
                shard_layout = None
                slice_config = None
            else:
                shard_layout = ttnn.TensorMemoryLayout.BLOCK_SHARDED
