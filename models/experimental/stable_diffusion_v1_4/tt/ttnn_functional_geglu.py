...
import ttnn
from ..tt.ttnn_functional_utility_functions import determine_blocking
...
class geglu:
    def __init__(self, device, parameters):
        ...
        self.grid_sizes = {8192: (5, 4), 2048: (5, 4), 512: (5, 4), 128: (5, 4)}
    ...
    def __call__(self, config, hidden_states):
        ...
        proj = ttnn.linear(
            ...
            program_config=None if use_dram_fallback else program_config,
            # memory_config=self.l1_interleaved_memory_config if interleaved_output else self.block_sharded_memory_config,
            memory_config=ttnn.DRAM_MEMORY_CONFIG if use_dram_fallback else self.block_sharded_memory_config,
        ...
        # For the 512x512 case (seq_len=4096 -> flattened 8192), the tensors are too large.
        if proj.shape[-2] == 8192:
            proj = ttnn.to_memory_config(proj, ttnn.DRAM_MEMORY_CONFIG)
            gate = ttnn.to_memory_config(gate, ttnn.DRAM_MEMORY_CONFIG)
        ret = ttnn.mul(
            proj,
            gate,
