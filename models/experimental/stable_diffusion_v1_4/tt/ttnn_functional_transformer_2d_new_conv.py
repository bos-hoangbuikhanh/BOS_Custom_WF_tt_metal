...
from ..tt.ttnn_functional_basic_transformer_block import basic_transformer_block
...
class transformer_2d_model:
    def __init__(self, device, parameters, batch_size, input_height, input_width, compute_kernel_config):
        ...
        self.norm_weight_fallback = ttnn.to_torch(parameters.norm.weight).to(torch.float32).reshape(-1)
        self.norm_bias_fallback = ttnn.to_torch(parameters.norm.bias).to(torch.float32).reshape(-1)
    ...
    def __call__(
        ...
        current_nhw = self.input_height * self.input_width

        hidden_states = ttnn.reshape(hidden_states, (self.batch_size, 1, current_nhw, in_channels))

        if current_nhw == 4096:
            hidden_states_torch = ttnn.to_torch(hidden_states).to(torch.float32)
            ...
            out_torch = torch.nn.functional.group_norm(
                hidden_states_torch,
                num_groups=norm_num_groups,
                weight=self.norm_weight_fallback,
                bias=self.norm_bias_fallback,
                eps=eps,
            )
        ...
        )  # Buffer::set_page_size is unsupported for BufferDistributionSpec!
