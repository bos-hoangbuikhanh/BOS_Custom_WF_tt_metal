...
import ttnn
from ..tt.ttnn_functional_utility_functions import (
    dealloc_input,
    determine_blocking,
...
class cross_attention:
    def __init__(self, device, parameters, seq_len):
        ...
        attention_mask_96 = torch.ones((1, 1, 32, 96)) * -1e9
        ...
        attention_mask_full = torch.zeros((1, 1, 32, seq_len))
        ...
        self.grid_sizes = {4096: (5, 4), 1024: (5, 4), 256: (5, 4), 64: (5, 4)}
        if self.fused_qkv:
            ...
            grid_sizes = {4096: (5, 4), 1024: (5, 4), 256: (5, 4), 64: (5, 4)}
        ...
        if have_time_sharded_attention:
            ...
            # ------
            # Ensure subblocking fits in destination registers (Max 8 tiles).
            # If width is 2, max height is 4. If width is 1, max height is 8.
            if out_subblock_w * out_subblock_h > 8:
        ...
        self.out_grid_sizes = {4096: (4, 4), 1024: (4, 4), 256: (4, 4), 64: (4, 4)}
    ...
    def sharded_attention(self, query, key, value, head_size, attn_type):
        ...
        # ------
        # We cap the max cores to 16 (4x4 grid) instead of 20 (5x4).
        # This ensures that standard SD sizes (16 heads * 4096 seq) divide evenly
        # by the core count (65536 / 16 = 4096 per core, which is tile aligned).
        # 65536 / 20 = 3276.8 (Not tile aligned -> Crash).

        if num_cores_softmax > 16:
            num_cores_softmax = 16
            end_grid = ttnn.CoreCoord(3, 3)
            compute_with_storage_grid_size = (4, 4)
        elif num_cores_softmax == 16:
            end_grid = ttnn.CoreCoord(3, 3)
            compute_with_storage_grid_size = (4, 4)
        else:
            end_grid = ttnn.CoreCoord(4, 3)
            compute_with_storage_grid_size = (5, 4)
            # Ensure we don't exceed available cores if num_cores_softmax is small
            if num_cores_softmax > 20:
                num_cores_softmax = 20
        # ------
        ...
        if use_mask:
            attention_scores = ttnn.scale_mask_softmax_in_place(
                ...
                1 / math.sqrt(head_size),
                attention_mask,
                # program_config=softmax_program_config, # FIXME: SCB issue
                is_causal_mask=False,
            )
        else:
        ...
        attention_scores = reshard_to(attention_scores, (4, 4), ttnn.TensorMemoryLayout.HEIGHT_SHARDED)
    ...
    def out(self, hidden_states):
        ...
        hidden_states = ttnn.linear(
            ...
            # program_config=program_config, #FIXME: SCB issue
            # memory_config=output_mem_config, #FIXME: SCB issue
            dtype=ttnn.bfloat8_b,
            compute_kernel_config=self.compute_kernel_config,
        )
    ...
    def __call__(
        ...
        if self.fused_qkv:
            ...
            if interleaved_out:
                ...
                qkv_out = reshard_to(qkv_out, (4, 2), ttnn.TensorMemoryLayout.BLOCK_SHARDED)
                query, key, value = ttnn.transformer.split_query_key_value_and_split_heads(
                    qkv_out,
                    memory_config=ttnn.L1_HEIGHT_SHARDED_MEMORY_CONFIG,
                    num_heads=heads,
                )
                query = reshard_to(query, (4, 4), ttnn.TensorMemoryLayout.HEIGHT_SHARDED)
                key = reshard_to(key, (4, 4), ttnn.TensorMemoryLayout.HEIGHT_SHARDED)
                value = reshard_to(value, (4, 4), ttnn.TensorMemoryLayout.HEIGHT_SHARDED)
            ...
            q_proj = ttnn.matmul(
                ...
                program_config=program_config,
                # memory_config=self.block_sharded_memory_config, #FIXME: SCB issues
                memory_config=ttnn.DRAM_MEMORY_CONFIG,
                dtype=ttnn.bfloat8_b,
                compute_kernel_config=self.compute_kernel_config,
            )
            ...
            end_core = ttnn.CoreCoord(3, 1) if self.seq_len != 64 else ttnn.CoreCoord(3, 1)
            grid_size = (4, 4) if self.seq_len != 64 else (4, 2)

            if self.seq_len == 4096:
                q_proj = ttnn.to_memory_config(q_proj, ttnn.DRAM_MEMORY_CONFIG)
                kv_proj = ttnn.to_memory_config(kv_proj, ttnn.DRAM_MEMORY_CONFIG)

                q_proj_torch = ttnn.to_torch(q_proj)
                kv_proj_torch = ttnn.to_torch(kv_proj)

                ttnn.deallocate(q_proj)
                ttnn.deallocate(kv_proj)

                # 3. Manual Head Splitting (Respecting Padded Weights)
                # The weights were padded (40->64) in __init__, so the output tensors
                # are already 64-dim per head. We must NOT slice off the end.

                # --- Handle Query ---
                # q_proj shape: [2, 1, 4096, 512]
                # 512 = 8 heads * 64 (40 data + 24 pad)
                # Reshape to separate heads: [2, 1, 4096, 8, 64]
                q_proj_torch = q_proj_torch.view(2, self.seq_len, heads, 64)

                # Permute to [2, 8, 4096, 64]
                query_torch = q_proj_torch.permute(0, 2, 1, 3).contiguous()

                # --- Handle Key & Value ---
                # kv_proj shape: [2, 1, 96, 1024]
                # 1024 = 16 heads * 64 (8 Heads K + 8 Heads V)
                # The Init function interleaves K and V heads: [K0, V0, K1, V1 ...]

                # Reshape to expose heads: [2, 1, 96, 8 (Pairs), 2 (KV), 64 (Dim)]
                kv_proj_torch = kv_proj_torch.view(2, 96, heads, 2, 64)

                # Split K and V
                k_torch = kv_proj_torch[:, :, :, 0, :]  # [2, 96, 8, 64]
                v_torch = kv_proj_torch[:, :, :, 1, :]  # [2, 96, 8, 64]

                # Process Key: [2, 96, 8, 64] -> [2, 8, 96, 64]
                k_torch = k_torch.permute(0, 2, 1, 3).contiguous()

                # Transpose K Heads (transpose_k_heads=True)
                # We need [2, 8, 64, 96] for the attention operation
                k_torch = k_torch.permute(0, 1, 3, 2).contiguous()

                # Process Value: [2, 96, 8, 64] -> [2, 8, 96, 64]
                v_torch = v_torch.permute(0, 2, 1, 3).contiguous()
        ...
        hidden_states = ttnn.transformer.concatenate_heads(
            hidden_states,
            # memory_config=ttnn.L1_BLOCK_SHARDED_MEMORY_CONFIG, #FIXME: SCB issues
            memory_config=ttnn.DRAM_MEMORY_CONFIG,
        )
