...
from models.bos_model.yolo.yolov10s.tt.c2fcib import TtnnC2fCIB
from models.bos_model.yolo.yolov10s.tt.common import Conv, concat, interleaved_to_sharded
...
class TtnnYolov10:
    def __init__(self, device, parameters, conv_pt):
        ...
        self.c2f_1 = TtnnC2f(
            shortcut=True, n=1, device=self.device, parameters=parameters.conv_args[2], conv_pt=conv_pt.model[2]
        )
        self.conv3 = Conv(device, parameters.conv_args[3], conv_pt.model[3], deallocate_activation=True)
        self.c2f_2 = TtnnC2f(
            shortcut=True, n=2, device=self.device, parameters=parameters.conv_args[4], conv_pt=conv_pt.model[4]
        )
        self.scdown_1 = TtnnSCDown(device=device, parameters=parameters.conv_args[5], conv_pt=conv_pt.model[5])
        self.c2f_3 = TtnnC2f(
            shortcut=True, n=2, device=self.device, parameters=parameters.conv_args[6], conv_pt=conv_pt.model[6]
        )
        self.scdown_2 = TtnnSCDown(device=device, parameters=parameters.conv_args[7], conv_pt=conv_pt.model[7])
        self.c2fcib_2 = TtnnC2fCIB(device=device, parameters=parameters.conv_args[8], conv_pt=conv_pt.model[8], n=1)
        ...
        self.c2f_4 = TtnnC2f(
            shortcut=False, n=1, device=self.device, parameters=parameters.conv_args[13], conv_pt=conv_pt.model[13]
        )
        self.c2f_5 = TtnnC2f(
            shortcut=False, n=1, device=self.device, parameters=parameters.conv_args[16], conv_pt=conv_pt.model[16]
        )
        self.conv4 = Conv(device, parameters.conv_args[17], conv_pt.model[17])
        self.c2f_6 = TtnnC2f(
            shortcut=False, n=1, device=self.device, parameters=parameters.conv_args[19], conv_pt=conv_pt.model[19]
        )
        self.scdown_3 = TtnnSCDown(device=device, parameters=parameters.conv_args[20], conv_pt=conv_pt.model[20])
        self.c2fcib_5 = TtnnC2fCIB(device=device, parameters=parameters.conv_args[22], conv_pt=conv_pt.model[22], n=1)
    ...
    def __call__(self, input_tensor):
        ...
        if branch1.get_layout() != ttnn.ROW_MAJOR_LAYOUT:
            branch1 = ttnn.sharded_to_interleaved(branch1, memory_config=ttnn.L1_MEMORY_CONFIG)
            branch1 = ttnn.to_dtype(ttnn.to_layout(branch1, ttnn.ROW_MAJOR_LAYOUT), ttnn.bfloat16)
        ...
        if features2.get_layout() != ttnn.ROW_MAJOR_LAYOUT:
            features2 = ttnn.sharded_to_interleaved(features2, memory_config=ttnn.L1_MEMORY_CONFIG)
            features2 = ttnn.to_dtype(ttnn.to_layout(features2, ttnn.ROW_MAJOR_LAYOUT), ttnn.bfloat16)

        fused_2 = concat(-1, False, up2, features2)
        ttnn.deallocate(up2)
        ttnn.deallocate(features2)

        features3 = self.c2f_5(fused_2)
