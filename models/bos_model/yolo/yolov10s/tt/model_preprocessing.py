...
import ttnn
from models.bos_model.yolo.yolov10s.reference.yolov10s import Conv, YOLOv10
...
def create_yolov10s_model_parameters(model: YOLOv10, input_tensor: torch.Tensor, device, inp_h=320, inp_w=320):
    ...
    strides = [8, 16, 32]
    feats = [inp_h // 8, inp_h // 16, inp_h // 32]  # value depend on res
    ...
    if "model" in parameters:
        ...
        parameters.model[9]["inp_h"] = inp_h
        parameters.model[9]["inp_w"] = inp_w
    else:
        parameters["inp_h"] = inp_h
        parameters["inp_w"] = inp_w
...
def create_yolov10_model_parameters_detect(
    model, input_tensor_1, input_tensor_2, input_tensor_3, device, weights_mesh_mapper=None, inp_h=320, inp_w=320
):
    parameters = preprocess_model_parameters(
        initialize_model=lambda: model,
        custom_preprocessor=create_custom_mesh_preprocessor(weights_mesh_mapper),
...
def create_yolov10s_input_tensors(
    device, batch_size=1, input_channels=3, input_height=224, input_width=224, use_sharding=True
):
    ...
    if use_sharding:
        ...
        input_mem_config = ttnn.create_sharded_memory_config(
            [n, c, h, w], ttnn.CoreGrid(x=4, y=4), ttnn.ShardStrategy.HEIGHT
        )
    else:
...
def create_yolov10s_input_tensors_submodules(
    device, batch_size=1, input_channels=3, input_height=320, input_width=320, input_dtype=ttnn.bfloat8_b
):
    torch_input_tensor = torch.randn(batch_size, input_channels, input_height, input_width)
