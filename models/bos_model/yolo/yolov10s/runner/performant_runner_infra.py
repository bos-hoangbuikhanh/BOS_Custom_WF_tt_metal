...
import ttnn
from models.bos_model.yolo.yolov10s.common import load_torch_model
from models.bos_model.yolo.yolov10s.tt.model_preprocessing import create_yolov10s_model_parameters
...
def load_coco_class_names():
    ...
    path = f"models/bos_model/yolov10x/demo/coco.names"
...
class YOLOv10PerformanceRunnerInfra:
    def __init__(
        ...
        self.torch_input_tensor = (
            torch.randn((self.batch_size * self.num_devices, 3, *self.resolution), dtype=torch.float32)
        ...
        self.torch_input_params = torch.randn((batch_size, 3, *self.resolution), dtype=torch.float32)
        self.parameters = create_yolov10s_model_parameters(
            self.torch_model,
            self.torch_input_params,
            device=self.device,
            inp_h=self.resolution[0],
            inp_w=self.resolution[1],
        )

        self.ttnn_yolov10s_model = TtnnYolov10(self.device, self.parameters, self.parameters)

    def run(self):
        self.output_tensor = self.ttnn_yolov10s_model(self.input_tensor)
    ...
    def validate(self, output_tensor=None, torch_output_tensor=None):
        ...
        logger.info(
            f"Yolov10s batch_size={self.batch_size}, act_dtype={self.act_dtype}, weight_dtype={self.weight_dtype}, PCC={self.pcc_passed}"
        )
