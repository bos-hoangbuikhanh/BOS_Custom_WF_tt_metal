...
from models.bos_model.qwen25_vl.tt.generator import create_submeshes
...
import ttnn
from models.bos_model.qwen25_vl.tt.common import hf_multimodal_encode
from models.bos_model.qwen25_vl.tt.generator import Generator
from models.bos_model.qwen25_vl.tt.model_config import CheckpointType, DecodersPrecision
...
def create_multimodal_model(
    ...
    from models.bos_model.qwen25_vl.tt.vision.qwen_e2e_model import TtQwen_Model
    ...
    if "Qwen2.5-VL" in tt_model_args.base_model_name:
        model = TtQwen_Model(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (0, True, 2, False, False),  # batch2-trace
        (0, True, 1, True, False),  # batch1-trace-with-text-prompts
        (0, False, 1, False, True),  # profiler # for tracy or l1 visualize
    ],
    ids=["batch1-notrace", "batch1-trace", "batch2-trace", "batch-one-trace-with-text-prompts", "profiler"],
)
@pytest.mark.parametrize(
    "data_parallel",
    [
...
def test_multimodal_demo_text(
    ...
    output_dir: Optional[str] = "models/bos_model/qwen25_vl/demo/outputs",  # <-- Optional save path
):
    """
    ...
    if print_to_file:
        ...
        output_path = os.path.join(output_dir, f"qwen25_vl_outputs.txt")
    ...
    with open("models/bos_model/qwen25_vl/demo/images/car.png", "rb") as f:
        trace_img_0 = PIL_Image.open(f).convert("RGB").resize((inp_h, inp_w))

    with open("models/bos_model/qwen25_vl/demo/images/dog.jpg", "rb") as f:
        trace_img_1 = PIL_Image.open(f).convert("RGB").resize((inp_h, inp_w))

    with open("models/bos_model/qwen25_vl/demo/images/pasta.jpg", "rb") as f:
        trace_img_2 = PIL_Image.open(f).convert("RGB").resize((inp_h, inp_w))

    with open("models/bos_model/qwen25_vl/demo/images/person.jpeg", "rb") as f:
