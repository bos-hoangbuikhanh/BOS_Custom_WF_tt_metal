...
import ttnn
from models.bos_model.qwen25_vl.tt.ccl import tt_all_gather, tt_all_reduce
from models.bos_model.qwen25_vl.tt.common import dequantize_gemm
from models.bos_model.qwen25_vl.tt.model_config import OpGroup, TensorGroup
from models.bos_model.qwen25_vl.tt.rmsnorm import RMSNorm as RMSNorm
...
class Attention(LightweightModule):
    def __init__(
        ...
        self.simplified_rms = True if configuration.base_model_name == "Qwen2.5-VL-7B" else False
        ...
        # )
        # print("wqkv_mem_config :", wqkv_mem_config)
        ...
        for i in range(self.num_devices_per_group):
            # Chunk weights
            if "AWQ" in configuration.model_name:
                wq_selected = torch.chunk(self.state_dict[f"{wq_str}.qweight"], self.num_devices_per_group, dim=0)[i]
                wk_selected = torch.chunk(self.state_dict[f"{wk_str}.qweight"], self.num_devices_per_group, dim=0)[i]
                wv_selected = torch.chunk(
                    dequantize_gemm(
                        self.state_dict[f"{wv_str}.qweight"],
                        self.state_dict[f"{wv_str}.qzeros"],
                        self.state_dict[f"{wv_str}.scales"],
                        bits=4,
                        group_size=128,
                    ),
        ...
        if "AWQ" in configuration.model_name:
            pt_wo = (
                dequantize_gemm(
                    self.state_dict[f"{wo_str}.qweight"],
                    self.state_dict[f"{wo_str}.qzeros"],
                    self.state_dict[f"{wo_str}.scales"],
                    bits=4,
                    group_size=128,
                )
                .unsqueeze(0)
    ...
    def nlp_concat_heads_decode(self, x, num_heads=32):
        """
        x: [S=1, B=batch, 32, head_dim]  (always padded to 32 heads)
        returns: [S=1, 1, B=batch, num_heads * head_dim]
        """
        if x.is_sharded():
            x = ttnn.sharded_to_interleaved(x, ttnn.DRAM_MEMORY_CONFIG)
            x = ttnn.to_layout(x, ttnn.ROW_MAJOR_LAYOUT)  # TILE layout does not have multicore implementation yet
        s, b, max_heads, head_dim = x.shape

        if b % 32 != 0:
            # Pad the ttnn input tensor
            x = ttnn.pad(x, ((0, 0), (0, 32 - b % 32), (0, 0), (0, 0)), value=0)

        assert x.shape[1] == 32, "Batch must be padded to 32"

        # shuffle → move num_heads into hidden dimension
        x = ttnn.reshape(x, (s, 1, 32, num_heads * head_dim))  # bsz must be 32
    ...
    def forward_decode(
        ...
        xqkv_fused_sharded = ttnn.linear(
            ...
            # program_config=ttnn.DRAM_MEMORY_CONFIG,   #self.model_config["XQKV_DECODE_PROGCFG"],  # FIXME
        ...
        attn_output_cat = self.nlp_concat_heads_decode(attn_output_11BH, num_heads=self.n_local_heads)
    ...
    def forward_prefill(
        ...
        xqkv_fused = ttnn.linear(
            ...
            # program_config=self.model_config["XQKV_PREFILL_PROGCFG"](seq_len), #FIXME
        )
