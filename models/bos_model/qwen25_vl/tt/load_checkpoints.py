...
def convert_hf_qkv_to_meta_format(loaded_weights, head_dim):
    ...
    converted_weights = {}
    for key, tensor in loaded_weights.items():
        if "q_proj.weight" in key or "k_proj.weight" in key or "q_proj.qweight" in key or "k_proj.qweight" in key:
            if "qweight" in key:
                # Dequantize AWQ weights before permutation
                qzeros = loaded_weights[key.replace("qweight", "qzeros")]
                scales = loaded_weights[key.replace("qweight", "scales")]
                tensor = dequantize_gemm(tensor, qzeros, scales, bits=4, group_size=128).transpose(0, 1)
        ...
        elif "q_proj.bias" in key or "k_proj.bias" in key or "q_proj.qbias" in key or "k_proj.qbias" in key:
