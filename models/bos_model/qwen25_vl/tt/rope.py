...
import ttnn
from models.bos_model.qwen25_vl.tt.common import RopeScaling, gather_cos_sin, get_rot_transformation_mat
...
class TTQwen2_5_VisionRotaryEmbedding:
    def __init__(self, device, dim: int, theta: float = 10000.0, mode="decode"):
        ...
        arange_indices = torch.arange(start=0, end=dim, step=2)
        exponent = arange_indices / dim
        pow_result = torch.pow(theta, exponent)
        recip = 1.0 / pow_result
        inv_freq = recip * 1.0
        self.inv_freq = ttnn.from_torch(inv_freq, dtype=ttnn.bfloat16, layout=ttnn.TILE_LAYOUT, device=device)
