...
import ttnn
from models.bos_model.qwen25_vl.tt.ccl import tt_all_reduce
from models.bos_model.qwen25_vl.tt.common import dequantize_gemm, pad_to_size
...
class MLP(LightweightModule):
    def __init__(
        ...
        if "AWQ" in args.model_name:
            torch_weight = lambda name: dequantize_gemm(
                self.state_dict[f"{state_dict_prefix}.{name}.qweight"],
                self.state_dict[f"{state_dict_prefix}.{name}.qzeros"],
                self.state_dict[f"{state_dict_prefix}.{name}.scales"],
                bits=4,
                group_size=128,
            )
        else:
