...
from models.bos_model.panoptic_deeplab.tt.tt_model import TtPanopticDeepLab, create_resnet_dtype_config
...
def run_panoptic_deeplab_demo(
    ...
    with torch.no_grad():
        pytorch_semantic_logits, pytorch_center_logits, pytorch_offset_logits, _ = pytorch_model.forward(
            pytorch_input.to(torch.float32)
        )
...
@pytest.mark.parametrize("model_category", [PANOPTIC_DEEPLAB])
def test_panoptic_deeplab_demo(device, output_dir, model_category, model_location_generator):
    ...
    for image in images:
        run_panoptic_deeplab_demo(device, image, weights_path, output_dir, model_category=model_category)
