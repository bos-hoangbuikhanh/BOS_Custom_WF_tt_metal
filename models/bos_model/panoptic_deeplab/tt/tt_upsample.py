...
class BilinearUpsampleMatmulTTNN(LightweightModule):
    ...
    def forward(self, img_ttnn):
        ...
        if img_ttnn.layout != ttnn.TILE_LAYOUT:
            if img_ttnn.dtype != ttnn.bfloat16:  # A0 requires typecast bfloat8b tile to row_major
                img_ttnn = ttnn.typecast(img_ttnn, dtype=ttnn.bfloat16)
        ...
        if self.output_channels_first:
            ...
            if Y_final.dtype != ttnn.bfloat16:  # A0 requires typecast bfloat8b tile to row_major
                Y_final = ttnn.typecast(Y_final, dtype=ttnn.bfloat16)
