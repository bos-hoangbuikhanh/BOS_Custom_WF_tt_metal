...
import ttnn
from models.bos_model.oft.reference.bbox import visualize_objects
...
from models.bos_model.oft.tt.model_preprocessing import create_decoder_model_parameters, create_OFT_model_parameters
...
NUM_ITER = int(os.getenv("NUM_ITER", "1"))
...
@pytest.mark.parametrize(
    ...
    # fmt: off
    [
       ( torch.float32, 0.944, 0.988, 0.999, 0.943),
    ],
    # fmt: on
)
@pytest.mark.parametrize("num_iter", [NUM_ITER])
@torch.no_grad()
def test_demo_inference(
    ...
    num_iter,
    intermediates_pcc_test=False,
):
    execution_date = time.strftime("%Y%m%d_%H%M%S", time.localtime())
    test_id = f"{execution_date}"
    ...
    intermediates, scores, pos_offsets, dim_offsets, ang_offsets = ref_model(
        input_tensor, calib, grid, return_intermediates=intermediates_pcc_test
    )
    ...
    iter = 0
    while iter < num_iter:
        signpost(header=f"{iter+1}th run")
        ...
        (tt_intermediates, layer_names), tt_scores, tt_pos_offsets, tt_dim_offsets, tt_ang_offsets = tt_model.forward(
            device, tt_input, tt_calib, tt_grid, return_intermediates=intermediates_pcc_test
        )
        _, _, _, _ = (
            tt_scores.cpu(),
            tt_pos_offsets.cpu(),
            tt_dim_offsets.cpu(),
            tt_ang_offsets.cpu(),
        )  # ensure all ops are done
        inference_time = time.time() - start_time
        print(f"TT OFTNet inference time: {inference_time:.4f} seconds ({1/inference_time:.2f} FPS)")
        ...
        inference_decode_time = time.time() - start_time
        print(f"TT OFTNet infer+decode time: {inference_decode_time:.4f} seconds ({1/inference_decode_time:.2f} FPS)")
        elapsed_times.append((inference_time, inference_decode_time))
        ...
        iter += 1
    print("===================================================")
    print("Elapsed times & FPS (inference only, infer+decode):")
    for i, times in enumerate(elapsed_times):
        print(f"  Iter[{i}] {times[0]:.4f} sec ({1/times[0]:.2f} FPS), {times[1]:.4f} sec ({1/times[1]:.2f} FPS)")
    print("===================================================")
    print("Average elapsed times (inference only, infer+decode):", np.mean(np.array(elapsed_times), axis=0), "seconds")
    print("Average FPS (inference only, infer+decode):", 1 / np.mean(np.array(elapsed_times), axis=0))

    if iter > 1:
        print(
            "Average elapsed times[1:] (inference only, infer+decode):",
            np.mean(np.array(elapsed_times[1:]), axis=0),
            "seconds",
        )
        print("Average FPS[1:] (inference only, infer+decode):", 1 / np.mean(np.array(elapsed_times[1:]), axis=0))
