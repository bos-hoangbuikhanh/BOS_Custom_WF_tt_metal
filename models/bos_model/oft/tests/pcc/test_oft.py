...
import pytest
import math
from models.bos_model.oft.tt.tt_oft import OFT
from models.bos_model.oft.reference.oft import OFT as ReferenceOFT
from tests.ttnn.utils_for_testing import check_with_pcc
from models.bos_model.oft.tt.model_preprocessing import create_OFT_model_parameters_oft
from models.bos_model.oft.reference.utils import get_abs_and_relative_error, make_grid
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ((1, 256, 48, 160), 256, 0.5, 4, 1 / 8, torch.float32,  False, 0.999, 0.933, 18),
        ((1, 256, 48, 160), 256, 0.5, 4, 1 / 8, torch.float32,   True, 0.999, 0.889, 18),
        ((1, 256, 48, 160), 256, 0.5, 4, 1 / 8, torch.bfloat16, False, 0.999, 0.742, 18),
        ((1, 256, 48, 160), 256, 0.5, 4, 1 / 8, torch.bfloat16,  True, 0.999, 0.707, 18),
        # feats16 {float32,bfloat16} x {use_precomputed_grid, no_use_precomputed_grid}
        ((1, 256, 24, 80), 256, 0.5, 4, 1 / 16, torch.float32,  False, 0.999, 0.672, 12),
        ((1, 256, 24, 80), 256, 0.5, 4, 1 / 16, torch.float32,   True, 0.999, 0.584, 12),
        ((1, 256, 24, 80), 256, 0.5, 4, 1 / 16, torch.bfloat16, False, 0.999, 0.398, 12),
        ((1, 256, 24, 80), 256, 0.5, 4, 1 / 16, torch.bfloat16,  True, 0.999, 0.381, 12),
        # feats32 {float32,bfloat16} x {use_precomputed_grid, no_use_precomputed_grid}
        ((1, 256, 12, 40), 256, 0.5, 4, 1 / 32, torch.float32,  False, 0.999, 0.420, 11),
        ((1, 256, 12, 40), 256, 0.5, 4, 1 / 32, torch.float32,   True, 0.999, 0.398, 11),
        ((1, 256, 12, 40), 256, 0.5, 4, 1 / 32, torch.bfloat16, False, 0.999, 0.255, 11),
        ((1, 256, 12, 40), 256, 0.5, 4, 1 / 32, torch.bfloat16,  True, 0.999, 0.247, 11),
        # fmt: on
    ],
    ids=[
