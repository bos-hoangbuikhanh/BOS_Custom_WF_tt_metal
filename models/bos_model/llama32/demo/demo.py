...
from models.bos_model.llama32.tt.model_config import DecodersPrecision, determine_device_name, parse_decoder_json
...
@pytest.mark.parametrize(
    ...
    [
        (  # Batch-1 run (Latency) - single user, small prompt
            "models/bos_model/llama32/demo/sample_prompts/input_data_questions_prefill_128.json",  # input_prompts
            True,  # instruct mode
            1,  # repeat_batches
            1024,  # max_seq_len
            1,  # batch_size
        ...
        (  # long-context-4k run - Single user, long prompt (may vary based on the model's tokenizer)
            "models/bos_model/llama32/demo/sample_prompts/input_data_long_1k.json",  # input_prompts
            True,  # instruct mode
            1,  # repeat_batches
            2 * 1024,  # max_seq_len
            1,  # batch_size
        ...
        (  # reasoning-1 - single user, small prompt, long thinking time
            "models/bos_model/llama32/demo/sample_prompts/input_data_questions_reasoning.json",  # input_prompts
            True,  # instruct mode
            1,  # repeat_batches
            512,  # max_seq_len
            1,  # batch_size
    ...
    ids=[
        "batch-one",  # latency
        "batch-16",  # throughput
        "long-context-1k",  # 1k context, max_seq_len=2k
...
def test_demo_text(
    ...
    # TODO: Remove this once OOM issue is resolved
    if os.environ.get("HF_MODEL") == "meta-llama/Llama-3.1-8B-Instruct" and ("long-context" in test_id):
        pytest.skip("Llama-3.1-8B-Instruct does not support long context yet.")
