...
import ttnn
from models.bos_model.qwen3_vl.tt.common import (
...
from models.bos_model.qwen3_vl.tt.load_checkpoints import (
    convert_hf_to_meta,
    convert_meta_to_hf,
            ...
            inst = cls(
                {
                    "TensorPrecision": {TensorGroup.FF1_FF3: PrecisionSetting.BFP4},
                    # "OpFidelity": {OpGroup.LI_FF1_FF3: MathFidelitySetting.LOFI},
                    "OpFidelity": {OpGroup.LI_FF1_FF3: MathFidelitySetting.HIFI2},
                }
            )
...
class ModelArgs:
    ...
    def __init__(
        ...
        if device is not None:  # Avoid issue with test_torch.py not having a device
            ...
            self.lm_head_core_grid = ttnn.CoreGrid(y=4, x=4)
            ...
            self.model_config["XQKV_PREFILL_PROGCFG"] = lambda seq_len: ttnn.MatmulMultiCoreReuseMultiCastProgramConfig(
                ...
                per_core_M=max(
                    ...
                    else math.ceil(seq_len / self.tile_size / 4),  # 4 rows # BOS
    ...
    def get_xqkv_prefill_mem_cfg(self, seq_len):
        return ttnn.create_sharded_memory_config(
            (((self.n_kv_heads // self.cluster_shape[1]) * seq_len // (4 * 4)), self.head_dim),
    ...
    def _set_vision_params(self, vision_config):
        ...
        if "Qwen2.5-VL" in self.base_model_name or "Qwen3-VL" in self.base_model_name:
            ...
            self.deepstack_visual_indexes = vision_config.get("deepstack_visual_indexes")
    ...
    def get_state_dict_prefix(self, module_name, layer_num, is_vision=False):
        ...
        if is_vision:
            layer_prefix = f"blocks.{layer_num}." if layer_num is not None else ""
        else:
            layer_prefix = f"layers.{layer_num}." if layer_num is not None else ""
        ...
        if self.dummy_weights:
            ...
            if self.from_hf_url:
                ...
                if "Qwen2.5-VL" in self.model_name:
                    ...
                    model = AutoModelForVision2Seq.from_pretrained(self.CKPT_DIR, torch_dtype="auto")
                else:
    ...
    def dram_shard_core_grid_for_k(self, k: int) -> Tuple[int, int]:
        ...
        return ttnn.CoreGrid(y=cols, x=rows)
    ...
    def dram_shard_core_grid_for_k_and_n(self, k: int, n: int) -> Tuple[int, int]:
        ...
        # return ttnn.CoreGrid(x=cols, y=rows)
        return ttnn.CoreGrid(x=rows, y=cols)
    ...
    def find_grid_k_n(self, K, N):
        ...
        # BOS grid configuration
        max_rows = 5
        max_cols = 4  # Maximum number of rows or columns
    ...
    def reference_vision_model(self):
        ...
        if "Qwen2.5-VL" or "Qwen3-VL" in self.model_name:
            layer = model.visual
    ...
    def reference_vision_deepstack_merger(self):
        """
        Loads the first Qwen3VLVisionPatchMerger module from the
        deepstack_merger_list for testing.
        This one uses use_postshuffle_norm=True.
        """
        model = self.reference_vision_transformer(wrap=False)

        # Correct Path: Qwen3VLForConditionalGeneration -> model -> visual -> deepstack_merger_list[0]
        layer = model.model.visual.deepstack_merger_list[0]
    ...
    def reference_vision_layernorm(self):
        ...
        layer = model.model.visual.blocks[0].norm1
        layer._load_state_dict = layer.load_state_dict
        layer.load_state_dict = lambda x: layer._load_state_dict(convert_vision_meta_to_hf(x, self.head_dim))
        return layer
