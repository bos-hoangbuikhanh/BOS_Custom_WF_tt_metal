...
from .performant_runner_infra import (
    DoubleStorage,
    InvocationBuffer,
    SSRPerformanceRunnerInfra,
)
from scripts.visualization import Visualizer, BEVMapLoader
...
class SSRPerformanceRunner:
    ...
    def _post_process(self, tt_result: Dict[str, Any], data: Dict[str, Any], **kwargs: Any) -> None:
        ...
        if kwargs.get("validate", False):
            ...
            _passed, _msg = op.compare_tensors(
                pt_fut_preds, tt_fut_preds, pcc=kwargs.get("pcc_threshold", 0.97)
            )
