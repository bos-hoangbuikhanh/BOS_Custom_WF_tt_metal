...
#
plugin = True
plugin_dir = 'tt/projects/mmdet3d_plugin/'
...
model = dict(
    ...
    img_backbone=dict(
        ...
        out_indices=(3,),
        # frozen_stages=1,
        norm_cfg=dict(type='BN_tt', requires_grad=False),
        norm_eval=True,
        style='pytorch',
        debug=False),
    img_neck=dict(
    ...
    pts_bbox_head=dict(
        ...
        valid_fut_ts=6,
        latent_decoder=dict(
            type='CustomTransformerDecoder_tt',
            num_layers=3,
            return_intermediate=False,
            transformerlayers=dict(
                type='BaseTransformerLayer_tt',
                attn_cfgs=[
                    dict(
                        type='MultiheadAttention_tt',
                        embed_dims=_dim_,
                        num_heads=8),
                ],
        ...
        transformer=dict(
            type='SSRPerceptionTransformer_tt',
            map_num_vec=map_num_vec,
            map_num_pts_per_vec=map_fixed_ptsnum_per_pred_line,
            ...
            embed_dims=_dim_,
            encoder=dict(
                type='BEVFormerEncoder_tt',
                num_layers=3,
                pc_range=point_cloud_range,
                num_points_in_pillar=4,
                return_intermediate=False,
                transformerlayers=dict(
                    type='BEVFormerLayer_tt',
                    attn_cfgs=[
                        dict(
                            type='TemporalSelfAttention_tt',
                            embed_dims=_dim_,
                            num_levels=1),
                        dict(
                            type='SpatialCrossAttention_tt',
                            pc_range=point_cloud_range,
                            deformable_attention=dict(
                                type='MSDeformableAttention3D_tt',
                                embed_dims=_dim_,
                                num_points=8,
                    ...
                    ffn_cfgs=dict(type='FFN_tt', embed_dims=_dim_),
        ...
        positional_encoding=dict(
            type='LearnedPositionalEncoding_tt',
            num_feats=_pos_dim_,
