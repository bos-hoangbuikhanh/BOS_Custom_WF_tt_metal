...
def test_resnetfeatures_forward(device, input_image_path, input_shape, layers, num_of_layers, expected_pcc):
    ...
    torch_tensor, _, _ = load_image(input_image_path, pad_hw=(input_shape[-2], input_shape[-1]))
    torch_tensor = torch_tensor[None]

    model = resnet18(
        pretrained=False, dtype=torch.float32, return_intermediates=True, frontend_mode=FrontendMode.REDUCED
    )
    model.eval()
