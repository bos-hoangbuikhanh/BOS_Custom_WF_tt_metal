...
from ...tt.model_preprocessing import create_decoder_model_parameters
from ...tests.common import GRID_RES, GRID_SIZE, Y_OFFSET, NMS_THRESH
...
def test_decode(
    ...
    tt_encoder = TTObjectEncoder(device, decoder_params, grid, nms_thresh=NMS_THRESH, return_intermediates=True)
    # Prepare TTNN inputs
    ...
    tt_outs, tt_intermediates, names, names_intermediates = tt_encoder.decode(
        device, scores_ttnn, pos_offsets_ttnn, dim_offsets_ttnn, ang_offsets_ttnn
    )
    ...
    for ref_out, tt_out, pcc, name in zip(
        ref_outs, tt_outs_torch, (pcc_peaks, pcc_scores, pcc_positions, pcc_dimensions, pcc_angles), names
    ):
