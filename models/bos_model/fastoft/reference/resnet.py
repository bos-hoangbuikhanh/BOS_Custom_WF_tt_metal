...
class ResNetFeatures(nn.Module):
    def __init__(
        ...
        self.layer3 = (
            self._make_layer(block, 256, layers[2], stride=2, dtype=dtype, return_intermediates=return_intermediates)
            if frontend_mode == FrontendMode.ORIGINAL
            else None
        )
        self.layer4 = (
            self._make_layer(block, 512, layers[3], stride=2, dtype=dtype, return_intermediates=return_intermediates)
            if frontend_mode == FrontendMode.ORIGINAL
            else None
        )
    ...
    def forward(self, x):
        ...
        if self.return_intermediates:
            ...
            feats16, i16 = self.layer3(feats8) if self.layer3 is not None else (None, None)
            feats32, i32 = self.layer4(feats16) if self.layer4 is not None else (None, None)
            return [ref_conv1, i4, i8, i16, i32, ref_conv1_maxpool], feats8, feats16, feats32
        else:
            feats4 = self.layer1(conv1)
            feats8 = self.layer2(feats4)
            feats16 = self.layer3(feats8) if self.layer3 is not None else None
            feats32 = self.layer4(feats16) if self.layer4 is not None else None
