...
"""
OFT-specific pipeline implementation with preprocessing/run pattern support.

This pipeline is adapted from the base tt_cnn pipeline to support models
...
from .oft_executor import (
    Executor,
    ModelExecutor,
    MultiCQModelOverlappedInputExecutor,
    MultiCQModelOverlappedDramInputExecutor,
...
class Pipeline:
    def __init__(self, executor: Executor):
        ...
        # if output tensor is in L1, and we are running non-traced version
        # someone needs to release memory
        self.deallocate_output_tensors = not hasattr(self.executor, "trace_id")
    ...
    def enqueue(self, host_inputs: list):
        ...
        if self.preallocated_output_tensors:
            ...
            for i, device_output_struct in enumerate(device_outputs):
                ...
                # if output tensor is in L1, and we are running non-traced version
                # someone needs to release memory
                if self.deallocate_output_tensors:
                    for d in device_output_struct if isinstance(device_output_struct, (list, tuple)) else [device_output_struct]:
                        if d.memory_config().buffer_type == ttnn.BufferType.L1 and d.is_allocated():
                            ttnn.deallocate(d, force=True)
...
def create_pipeline_from_config(
    ...
    if config.num_command_queues == 1:
        ...
        if not config.use_trace:
            ...
            if l1_input_memory_config is not None:
                ...
                executor = MultiCQModelOverlappedDramInputExecutor(
                    model,
                    device,
                    dram_input_memory_config=dram_input_memory_config,
                )
        else:
            # Traced multi-CQ executor
            if config.all_transfers_on_separate_command_queue:
                raise ValueError(
                    "MultiCQTracedModelPipelinedIOExecutor not supported in OFT pipeline (requires separate implementation)"
                )
