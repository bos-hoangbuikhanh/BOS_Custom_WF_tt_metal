...
def test_oftnet(
    ...
    input_tensor, _, _ = load_image(input_image_path, pad_hw=(H_PADDED, W_PADDED), dtype=torch.float32)
    input_tensor = input_tensor[None].to(torch.float32)
    ...
    for i, (ref_out, test_out, layer_name) in enumerate(zip(intermediates, test_intermediates, layer_names)):
        if "bbox" in layer_name:
            ...
            continue
        if ref_out is None or test_out is None:
