...
from models.bos_model.ufld_v2.demo.demo_utils import LaneEval, run_test_tusimple
from models.bos_model.ufld_v2.reference.ufld_v2_model import TuSimple34
from models.bos_model.ufld_v2.runner.performant_runner import UFLDPerformantRunner
from models.bos_model.ufld_v2.runner.performant_runner_infra import load_torch_model
...
def run_ufld_v2_dataset_inference(
    ...
    # dataset_path = "models/bos_model/ufld_v2/demo/image_data"
    dataset_path = os.path.join(cfg.data_root, "image_data")
    if not os.path.exists(dataset_path):
        # subprocess.run(["python3", "models/bos_model/ufld_v2/demo/data_download.py", "--tusimple"], check=True)
        subprocess.run(["python3", os.path.join(cfg.data_root, "data_download.py"), "--tusimple"], check=True)
    cfg.row_anchor = np.linspace(160, 710, cfg.num_row) / 720
    cfg.col_anchor = np.linspace(0, 1, cfg.num_col)
    run_test_tusimple(
        reference_model,
    ...
    gt_file_path = os.path.join(cfg.data_root, "image_data/tusimple/test_label_till_nimages.json")
    os.makedirs(os.path.dirname(gt_file_path), exist_ok=True)
    input_file = os.path.join(cfg.data_root, "image_data/tusimple/test_label.json")
...
@pytest.mark.parametrize(
    ...
    [
        (1, 3, 320, 800, 100, True, "reference_model_results_dataset", "ttnn_model_results_dataset"),
    ],
)
@pytest.mark.parametrize(
    "use_pretrained_weight",
