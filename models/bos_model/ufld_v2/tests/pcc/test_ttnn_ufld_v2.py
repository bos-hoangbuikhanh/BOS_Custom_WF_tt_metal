...
from models.bos_model.ufld_v2.reference.ufld_v2_model import BasicBlock, CULane34, TuSimple34
...
from models.bos_model.ufld_v2.ttnn.ttnn_ufld_v2 import TtnnUFLDv2
...
def custom_preprocessor_whole_model(model, name, mesh_mapper=None):
    parameters = {}
    if isinstance(model, (TuSimple34, CULane34)):
        ...
        if isinstance(model, CULane34):
            parameters["cls"]["layernorm"] = {}
            parameters["cls"]["layernorm"]["weight"] = ttnn.from_torch(
                model.cls[0].weight, dtype=ttnn.bfloat16, mesh_mapper=mesh_mapper
            )
            parameters["cls"]["layernorm"]["bias"] = ttnn.from_torch(
                model.cls[0].bias, dtype=ttnn.bfloat16, mesh_mapper=mesh_mapper
            )
...
def test_ufld_v2_model(device, batch_size, input_channels, height, width, use_pretrained_weight, min_channels=8):
    ...
    if use_pretrained_weight:
        weights_path = "models/bos_model/ufld_v2/tusimple_res34.pth"
        if not os.path.exists(weights_path):
            os.system("bash models/bos_model/ufld_v2/weights_download.sh --tusimple34")
    ...
    ttnn_input_tensor = ttnn_input_tensor.to(device, dram_config)
    parameters = preprocess_model_parameters(
        initialize_model=lambda: torch_model,
    ...
    ttnn_model = TtnnUFLDv2(conv_args=parameters.conv_args, conv_pth=parameters, device=device, model_type="tusimple")
    torch_output, pred_list = torch_model(torch_input_tensor)
    ttnn_output = ttnn_model(
        input=ttnn_input_tensor, batch_size=batch_size, grid_size=(5, 4), linear_memory_config="L1_INTERLEAVED"
    )

    ##=== to save the graph ===#
    # with ttnn.tracer.trace():
    #    ttnn_output = ttnn_model(input=ttnn_input_tensor, batch_size=batch_size, grid_size=(5, 4), linear_memory_config='L1_INTERLEAVED')
    # ttnn.tracer.visualize(ttnn_output, file_name="ttnn_model_ufld_v2.svg")

    ttnn_output = ttnn.to_torch(ttnn_output).squeeze(dim=0).squeeze(dim=0)
    pcc_passed, pcc_message = assert_with_pcc(torch_output, ttnn_output, pcc=0.989)
    logger.info(f"ufld_v2 - batch_size={batch_size}, PCC={pcc_message}")
...
@pytest.mark.parametrize("device_params", [{"l1_small_size": UFLD_V2_L1_SMALL_SIZE}], indirect=True)
def test_ufld_v2_conv1_only(device, batch_size, input_channels, height, width, min_channels=16):
    # PyTorch model
    torch_model = TuSimple34(input_height=height, input_width=width)
    torch_model.to(torch.bfloat16)
    torch_model.eval()
    ...
    # conv1
    torch_conv1 = torch_model.res_model.conv1
    torch_bn1 = torch_model.res_model.bn1

    torch_input_tensor = torch.randn((batch_size, input_channels, height, width), dtype=torch.bfloat16)

    torch_output = torch_conv1(torch_input_tensor)
    torch_output = torch_bn1(torch_output)
    ...
    ttnn_input_tensor = ttnn.pad(ttnn_input_tensor, ((0, 0), (0, channel_padding_needed), (0, 0), (0, 0)), value=0.0)
    ttnn_input_tensor = ttnn.permute(ttnn_input_tensor, (0, 2, 3, 1))
    ttnn_input_tensor = ttnn.reshape(ttnn_input_tensor, (1, 1, n * h * w, min_channels))
    ...
    ttnn_conv1 = TtnnUFLDV2Conv2D(
        parameters.conv_args.conv1,
        parameters.res_model.conv1,
        device=device,
    ...
    ttnn_output, out_h, out_w = ttnn_conv1(ttnn_input_tensor)

    ttnn_output = ttnn.to_torch(ttnn_output)
    ttnn_output = ttnn_output.reshape(batch_size, out_h, out_w, -1)
    ttnn_output = ttnn_output.permute(0, 3, 1, 2)
...
@pytest.mark.parametrize("device_params", [{"l1_small_size": UFLD_V2_L1_SMALL_SIZE}], indirect=True)
def test_ufld_v2_culane_model(device, batch_size, input_channels, height, width, use_pretrained_weight, min_channels=8):
    torch_model = CULane34(input_height=height, input_width=width)
    torch_model.to(torch.bfloat16)
    torch_model.eval()
    torch_input_tensor = torch.randn((batch_size, input_channels, height, width), dtype=torch.bfloat16)
    torch_output = torch_model(torch_input_tensor)
    ...
    if use_pretrained_weight:
        weights_path = "models/bos_model/ufld_v2/culane_res34.pth"
        if not os.path.exists(weights_path):
            os.system("bash models/bos_model/ufld_v2/weights_download.sh --culane34")
        if os.path.exists(weights_path):
            ...
            for key, value in state_dict["model"].items():
                ...
                if key.startswith("model."):
                    ...
                    new_key = key
                elif key == "pool.weight" or key == "pool.bias":
                    new_key = key
                else:
                    new_key = key
    ...
    ttnn_model = TtnnUFLDv2(conv_args=parameters.conv_args, conv_pth=parameters, device=device, model_type="culane")
    torch_output, pred_list = torch_model(torch_input_tensor)
    ttnn_output = ttnn_model(input=ttnn_input_tensor, batch_size=batch_size)
    ttnn_output = ttnn.to_torch(ttnn_output).squeeze(dim=0).squeeze(dim=0)

    pcc_passed, pcc_message = assert_with_pcc(torch_output, ttnn_output, pcc=0.985)
    logger.info(f"ufld_v2 - batch_size={batch_size}, PCC={pcc_message}")
