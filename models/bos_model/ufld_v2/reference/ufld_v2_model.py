...
class CULane34(nn.Module):
    def __init__(self, input_height=320, input_width=1600):
        super(CULane34, self).__init__()
        ...
        self.input_height = input_height
        self.input_width = input_width
        self.input_dim = input_height // 32 * input_width // 32 * 8  # equals 4000 for default 320x1600 input
        self.fc_norm = False
        self.res_model = CustomResNet34(BasicBlock, [3, 4, 6, 3])

        self.cls = torch.nn.Sequential(
            torch.nn.LayerNorm(self.input_dim),  # Added LayerNorm for CULane
            torch.nn.Linear(self.input_dim, mlp_mid_dim),
            torch.nn.ReLU(),
            torch.nn.Linear(mlp_mid_dim, self.total_dim),
        )
