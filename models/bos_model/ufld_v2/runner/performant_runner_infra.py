...
import ttnn
from models.bos_model.ufld_v2.common import load_torch_model
...
def load_ttnn_model(
    ...
    ttnn_model = TtnnUFLDv2(conv_args=parameters.conv_args, conv_pth=parameters, device=device, model_type=model_type)
    return ttnn_model
...
class UFLDPerformanceRunnerInfra:
    def __init__(
        ...
        self.torch_model = load_torch_model(model_location_generator, use_pretrained_weight=True, model_type=model_type)
        self.ttnn_ufld_v2_model = load_ttnn_model(
            self.device,
            self.torch_model,
            self.torch_input_tensor_per_device,
    ...
    def setup_dram_interleaved_input(self, device, torch_input_tensor=None):
        torch_input_tensor = self.torch_input_tensor if torch_input_tensor is None else torch_input_tensor
    ...
    ## Keep existing methods and add a helper for configuring double buffering
    # def setup_dram_interleaved_double_buffer(self, device, torch_input_tensor=None):
    #    """Configure DRAM interleaved double buffering"""
    #    torch_input_tensor = self.torch_input_tensor if torch_input_tensor is None else torch_input_tensor
    #    tt_inputs_host = ttnn.from_torch(torch_input_tensor, dtype=ttnn.bfloat16, layout=ttnn.ROW_MAJOR_LAYOUT)

    #    # Allocate two DRAM buffers
    #    dram_buffers = [
    #        ttnn.allocate_tensor_on_device(
    #            tt_inputs_host.shape, tt_inputs_host.dtype, tt_inputs_host.layout, device, ttnn.DRAM_MEMORY_CONFIG
    #        ),
    #        ttnn.allocate_tensor_on_device(
    #            tt_inputs_host.shape, tt_inputs_host.dtype, tt_inputs_host.layout, device, ttnn.DRAM_MEMORY_CONFIG
    #        ),
    #    ]
    #    return tt_inputs_host, dram_buffers

    def run(self):
        self.output_tensor_1 = self.ttnn_ufld_v2_model(
            input=self.input_tensor, grid_size=(5, 4), linear_memory_config="L1_INTERLEAVED"
        )
