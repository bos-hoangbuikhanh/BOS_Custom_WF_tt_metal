...
def perspective(matrix, vector):
    ...
    # torch.save(homogenous[..., :-1], "~/oft/dumps/torch_1.pt")
    # torch.save(homogenous[..., [-1]], "~/oft/dumps/torch_2.pt")
...
class OFT(nn.Module):
    ...
    def forward(self, features, calib, grid):
        ...
        # normalized = img_corners / img_size
        # scaled = 2 * normalized
        # shifted = scaled - 1
        # norm_corners = shifted.clamp(-1,1)
        ...
        diag_sum = top_left + btm_right
        subtract_top_right = diag_sum - top_right
        vox_feats = subtract_top_right - btm_left

        vox_feats = vox_feats / area
        vox_feats = vox_feats * visible.float()

        vox_feats = vox_feats.permute(0, 3, 1, 2).flatten(0, 1).flatten(1, 2)
        ortho_feats = self.conv3d(vox_feats)
        ortho_feats = ortho_feats.view(batch, depth, width, -1)
