...
@pytest.mark.parametrize("times", list(range(10, 100, 5)))
@pytest.mark.parametrize(
    "shape",
    [
        (1, 1, 25281, 256),
...
def test_to_and_from_loop(device, times, shape):
    write_tensors = []
    write_ttnn_tensors = []
    for _ in range(times):
        torch_input_tensor = torch.rand(shape, dtype=torch.bfloat16)
        write_tensors.append(torch_input_tensor)
    for i in range(times):
        write_ttnn_tensors.append(
            ttnn.from_torch(
                write_tensors[i], layout=ttnn.TILE_LAYOUT, memory_config=ttnn.DRAM_MEMORY_CONFIG, device=device
            )
        )
    print("from_torch done")
    read_tensors = []
    for i in range(times):
        read_tensors.append(ttnn.to_torch(write_ttnn_tensors[i]))
        print(f"to_torch[{i}] done")
    print("to_torch done")
    for i in range(times):
        assert torch.allclose(write_tensors[i], read_tensors[i])
