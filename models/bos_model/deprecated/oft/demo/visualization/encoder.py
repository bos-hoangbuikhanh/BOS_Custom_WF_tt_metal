...
from models.bos_model.oft.demo.visualization import utils
...
class ObjectEncoder(object):
    ...
    def encode_batch(self, objects, grids):
        ...
        batch_encoded = [self.encode(objs, grid) for objs, grid in zip(objects, grids)]
    ...
    def encode(self, objects, grid):
        ...
        classids = torch.tensor([self.classnames.index(obj.classname) for obj in objects], device=grid.device)
        ...
        mask, indices = self._assign_to_grid(classids, positions, dimensions, angles, grid)
    ...
    def _assign_to_grid(self, classids, positions, dimensions, angles, grid):
        ...
        local_grid = utils.rotate(centers - positions.view(-1, 1, 1, 3), -angles.view(-1, 1, 1)) / dimensions.view(
            -1, 1, 1, 3
        )
        ...
        class_mask = classids.view(-1, 1) == torch.arange(len(self.classnames)).type_as(classids)
    ...
    def _encode_heatmaps(self, classids, positions, grid):
        centers = (grid[1:, 1:, [0, 2]] + grid[:-1, :-1, [0, 2]]) / 2.0
    ...
    def decode(self, heatmaps, pos_offsets, dim_offsets, ang_offsets, grid):
        ...
        for score, cid, pos, dim, ang in zip(scores, classids, positions, dimensions, angles):
            objects.append(utils.ObjectData(self.classnames[cid], pos, dim, ang, score))
