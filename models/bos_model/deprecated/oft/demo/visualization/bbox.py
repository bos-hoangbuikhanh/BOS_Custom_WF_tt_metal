...
def draw_bbox3d_cv(img_bgr, obj, calib, color=(0, 255, 0), thickness=2):
...
def visualize_objects_cv(image, calib, objects, cmap_name="tab20", thickness=2):
    ...
    cmap = cm.get_cmap(cmap_name, max(len(objects), 1))
    for i, obj in enumerate(objects):
        color = _mpl_to_bgr255(cmap(i))
        draw_bbox3d_cv(img_bgr, obj, calib, color=color, thickness=thickness)
