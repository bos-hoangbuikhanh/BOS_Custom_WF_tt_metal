...
class KittiObjectDataset(Dataset):
    def __init__(self, kitti_root, split="train", grid_size=(80.0, 80.0), grid_res=0.5, y_offset=1.74):
        # Get the root directory containing object detection data

        kitti_split = "training" if split == "train" else "testing"
        assert kitti_split == "testing", "Only testing is supported at the moment"

        self.root = os.path.join(kitti_root, "object", kitti_split)
        split_file = "models/bos_model/oft/demo/visualization/splits/test.txt"
        self.indices = read_split(split_file)

        # Make grid
        self.grid = utils.make_grid(grid_size, (-grid_size[0] / 2.0, y_offset, 0.0), grid_res)
