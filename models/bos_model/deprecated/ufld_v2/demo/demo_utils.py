...
def get_test_loader(batch_size, data_root, dataset, distributed, crop_ratio, train_width, train_height, n_images=-1):
    if dataset == "Tusimple":
    ...
    if n_images > 0 and len(test_dataset) > n_images:
        # Manually truncate the dataset to the desired size
        test_dataset.image_files = test_dataset.image_files[:n_images]
...
def run_test_tusimple(
    ...
    n_images=100,
    is_overlay=False,
    json_path="models/bos_model/ufld_v2/demo/image_data/test_label.json",
    is_eval=False,
    ...
    for data in tqdm(loader, desc="Processing images", ncols=n_images):
        ...
        if exp_name.startswith("reference_model"):
            ...
            # FIXME: Modify this block of code to run for both, with and without trace

            # --- Comment this block of code when running demo WITH trace
