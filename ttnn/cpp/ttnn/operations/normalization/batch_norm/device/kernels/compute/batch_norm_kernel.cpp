...
// [BOS-ARCH-006] Workaround llk hang issue
#define BOS_HANG_WA true
namespace NAMESPACE {
...
ALWI void batchnorm_bcast_tiles(
    ...
    // [BOS-ARCH-006] Workaround llk hang issue
    cb_pop_front<BOS_HANG_WA>(cb_batch_var, onetile);
    ...
    for (uint32_t j = tile_start; j < freq; ++j) {
        ...
        // [BOS-ARCH-006] Workaround llk hang issue
        cb_pop_front<BOS_HANG_WA>(cb_other, onetile);
        ...
        if (weight_has_value) {
            ...
            // [BOS-ARCH-006] Workaround llk hang issue
            cb_pop_front<BOS_HANG_WA>(cb_affine_or_out, 1);
        ...
        if (bias_has_value) {
            ...
            // [BOS-ARCH-006] Workaround llk hang issue
            cb_pop_front<BOS_HANG_WA>(cb_tmp_1, onetile);
            cb_push_back(cb_output_0, onetile);
        }
    }
    // [BOS-ARCH-006] Workaround llk hang issue
    cb_pop_front<BOS_HANG_WA>(cb_bcast, onetile);
    cb_pop_front<BOS_HANG_WA>(cb_den, onetile);
    if (weight_has_value) {
        // [BOS-ARCH-006] Workaround llk hang issue
        cb_pop_front<BOS_HANG_WA>(cb_weight, onetile);
    }
    if (bias_has_value) {
        // [BOS-ARCH-006] Workaround llk hang issue
        cb_pop_front<BOS_HANG_WA>(cb_bias, onetile);
    }
}
...
void MAIN {
    ...
    // [BOS-ARCH-006] Workaround llk hang issue
    cb_pop_front<BOS_HANG_WA>(cb_eps, onetile);
}
}  // namespace NAMESPACE
