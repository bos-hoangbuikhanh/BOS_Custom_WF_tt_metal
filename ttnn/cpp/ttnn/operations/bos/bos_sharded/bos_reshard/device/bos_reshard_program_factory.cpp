...
#include "bos_reshard_program_factory.hpp"
...
#include "ttnn/operations/bos/bos_sharded/bos_sharded_common.hpp"
...
enum class BosReshardStridesInRange { ALL_STRIDES, FIRST_HALF, SECOND_HALF };
...
std::vector<uint32_t> get_runtime_args_for_given_ranges(
    ...
    const BosReshardStridesInRange reshard_strides_in_range = BosReshardStridesInRange::ALL_STRIDES) {
    ...
    for (uint32_t range_id = starting_range; range_id < ending_range; range_id++) {
        ...
        uint32_t start_data;
        if (reshard_strides_in_range == BosReshardStridesInRange::ALL_STRIDES) {
            num_strides = ps.num_strides;
            start_core_x = ps.start_core.x;
            start_core_y = ps.start_core.y;
            start_data = ps.start_data;
        } else {
            if (reshard_strides_in_range == BosReshardStridesInRange::FIRST_HALF) {
...
// Template function for resharding tensors with the same width but unaligned memory
template <bool is_reader>
operation::ProgramWithCallbacks bos_reshard_multi_core_same_width_unalign(const Tensor& input, Tensor& output) {
    auto device = input.device();
    bool unaligned = true;

    tt::tt_metal::Program program{};

    // Determine the local and remote tensors based on whether this is a reader or writer
    const auto& local_tensor = is_reader ? output : input;
    const auto& remote_tensor = is_reader ? input : output;

    // Extract shard specifications and core grids
    ...
    constexpr uint32_t cb_scratch_1_index = tt::CBIndex::c_1;
    constexpr uint32_t cb_scratch_2_index = tt::CBIndex::c_2;
    ...
    // Determine unit sizes and shard dimensions
    ...
    const std::string kernel_name =
        is_reader
            ? "ttnn/cpp/ttnn/operations/bos/bos_sharded/device/kernels/dataflow/bos_reshard_same_width_reader.cpp"
            : "ttnn/cpp/ttnn/operations/bos/bos_sharded/device/kernels/dataflow/bos_reshard_same_width_writer.cpp";
    ...
    // Configure circular buffers for local and scratch memory
    ...
    tt::tt_metal::CircularBufferConfig cb_scratch_1_config =
        tt::tt_metal::CircularBufferConfig(
            local_units_per_shard / 4 * remote_unit_size_padded, {{cb_scratch_1_index, data_format}})
            .set_page_size(cb_scratch_1_index, unit_size);
    tt::tt_metal::CreateCircularBuffer(program, all_cores, cb_scratch_1_config);

    tt::tt_metal::CircularBufferConfig cb_scratch_2_config =
        tt::tt_metal::CircularBufferConfig(
            local_units_per_shard / 4 * remote_unit_size_padded, {{cb_scratch_2_index, data_format}})
            .set_page_size(cb_scratch_2_index, unit_size);
    tt::tt_metal::CreateCircularBuffer(program, all_cores, cb_scratch_2_config);

    // Initialize variables for remote core traversal
    ...
    for (const auto& core : local_cores) {
        uint32_t local_units_this_core = std::min(local_units_left, local_units_per_shard);
        local_units_left -= local_units_this_core;
        uint32_t local_units_per_transfer_max = tt::div_up(tt::div_up(local_units_this_core, kernels.size()), 2);
        uint32_t local_start_offset = 0;

        // Prepare kernel arguments for each kernel
        for (const auto& kernel : kernels) {
            std::vector<uint32_t> kernel_args = {remote_address, 0, 0};
            kernel_args[1] = local_start_offset;  // always start at beginning of local shard
            uint32_t num_transfers = 0;
            for (int k = 0; k < 2; k++) {
                uint32_t local_units_to_transfer = std::min(local_units_this_core, local_units_per_transfer_max);
                    ...
                    while (local_units_to_transfer > 0) {
                        ...
                        kernel_args.insert(
                            ...
                            {bank_id,
                             ...
                             units_to_transfer});
                        local_units_this_core -= units_to_transfer;
            ...
            SetRuntimeArgs(program, kernel, core, kernel_args);
        }
    }

    // Callback to override runtime arguments dynamically
...
// Template function for resharding tensors with the same width and aligned memory
template <bool is_reader>
operation::ProgramWithCallbacks bos_reshard_multi_core_same_width_align(const Tensor& input, Tensor& output) {
    auto device = input.device();
    ...
    // Configure circular buffer for local memory
    tt::tt_metal::CircularBufferConfig cb_config =
        tt::tt_metal::CircularBufferConfig(total_size, {{cb_index, data_format}})
            .set_page_size(cb_index, unit_size)
            .set_globally_allocated_address(*local_tensor.buffer());
...
// Template function for resharding tensors with the same width
template <bool is_reader>
operation::ProgramWithCallbacks bos_reshard_multi_core_same_width(const Tensor& input, Tensor& output) {
    const auto& local_tensor = is_reader ? output : input;
    const auto& remote_tensor = is_reader ? input : output;
    ...
    if (remote_unit_size_padded != unit_size || local_unit_size_padded != unit_size) {
        return bos_reshard_multi_core_same_width_unalign<is_reader>(input, output);
    } else {
        return bos_reshard_multi_core_same_width_align<is_reader>(input, output);
    }
}

operation::ProgramWithCallbacks bos_reshard_multi_core_generic(const Tensor& input, Tensor& output) {
    auto device = input.device();
    ...
    tt::tt_metal::KernelHandle kernel_id_0 = tt::tt_metal::CreateKernel(
        program,
        "ttnn/cpp/ttnn/operations/bos/bos_sharded/device/kernels/dataflow/bos_reshard_reader.cpp",
        all_cores,
...
template <bool is_reader>
operation::ProgramWithCallbacks bos_reshard_multi_core_same_height(const Tensor& input, Tensor& output) {
    auto device = input.device();
    ...
    const std::string kernel_name =
        is_reader
            ? "ttnn/cpp/ttnn/operations/bos/bos_sharded/device/kernels/dataflow/bos_reshard_same_height_reader.cpp"
            : "ttnn/cpp/ttnn/operations/bos/bos_sharded/device/kernels/dataflow/bos_reshard_same_height_writer.cpp";
    ...
    auto [runtime_args_for_each_core, total_num_sticks, local_stride_bytes, remote_stride_bytes] =
        bos_compute_width_sharding_reshard_segments(
...
operation::ProgramWithCallbacks bos_reshard_multi_core(const Tensor& input, Tensor& output) {
    if (input.memory_config().memory_layout() == TensorMemoryLayout::HEIGHT_SHARDED &&
        ...
        if (output.memory_config().buffer_type() == BufferType::L1) {
            return bos_reshard_multi_core_same_width<true>(input, output);
        } else {
            return bos_reshard_multi_core_same_width<false>(input, output);
        }
    } else if (
        ...
        if (output.memory_config().buffer_type() == BufferType::L1) {
            return bos_reshard_multi_core_same_height<true>(input, output);
        } else {
            return bos_reshard_multi_core_same_height<false>(input, output);
        }
    } else {
        return bos_reshard_multi_core_generic(input, output);
    }
}
